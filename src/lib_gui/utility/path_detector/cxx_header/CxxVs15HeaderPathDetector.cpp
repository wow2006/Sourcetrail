...
std::vector<FilePath> CxxVs15HeaderPathDetector::doGetPaths() const {
...

  {
    const std::vector<FilePath> expandedPaths =
        FilePath(L"%ProgramFiles(x86)%/Microsoft Visual Studio/Installer/vswhere.exe").expandEnvironmentVariables();
    if (!expandedPaths.empty()) {
      const utility::ProcessOutput out = utility::executeProcess(
          expandedPaths.front().wstr(), {L"-latest", L"-property", L"installationPath"}, FilePath(), false, 10000);
      if (out.exitCode == 0) {
  ...
  if (!headerSearchPaths.empty()) {
    std::vector<FilePath> windowsSdkHeaderSearchPaths =
        utility::getWindowsSdkHeaderSearchPaths(APPLICATION_ARCHITECTURE_X86_32);
