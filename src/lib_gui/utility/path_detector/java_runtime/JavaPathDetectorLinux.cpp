...
FilePath JavaPathDetectorLinux::readLink(const FilePath& path) const {
  const utility::ProcessOutput out =
      utility::executeProcess(L"readlink", std::vector<std::wstring>{L"-f", path.wstr()});
...
bool JavaPathDetectorLinux::checkVersion(const FilePath& path) const {
  ...
  return ((out.exitCode == 0) && (out.output.find(utility::decodeFromUtf8(m_javaVersion)) != std::string::npos));
}
