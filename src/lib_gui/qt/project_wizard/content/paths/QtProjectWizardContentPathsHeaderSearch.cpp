  ...
  if (!m_indicateAsAdditional) {
    {
      ...
      connect(detectionButton, &QPushButton::clicked, this,
    ...
    }
    {
      ...
      connect(validationButton, &QPushButton::clicked, this,
...
void QtProjectWizardContentPathsHeaderSearch::detectIncludesButtonClicked() {
  ...
  connect(m_pathsDialog.get(), &QtPathListDialog::finished, this,
          &QtProjectWizardContentPathsHeaderSearch::finishedSelectDetectIncludesRootPathsDialog);
  connect(m_pathsDialog.get(), &QtPathListDialog::canceled, this,
      ...
      std::vector<IncludeDirective> unresolvedIncludes;
      {
        ...
        }
        {
          ...
          unresolvedIncludes = IncludeProcessing::getUnresolvedIncludeDirectives(
              sourceFilePaths, utility::toSet(indexedFilePaths), utility::toSet(headerSearchPaths),
              static_cast<size_t>(log2(sourceFilePaths.size())), [&](const float progress) {
                dialogView->showProgressDialog(L"Processing",
      ...
      std::set<FilePath> detectedHeaderSearchPaths;
      {
        ...
        }
        {
          ...
          detectedHeaderSearchPaths = IncludeProcessing::getHeaderSearchDirectories(
              sourceFilePaths, utility::toSet(searchedPaths), utility::toSet(headerSearchPaths),
              static_cast<size_t>(log2(sourceFilePaths.size())), [&](const float progress) {
                dialogView->showProgressDialog(L"Processing",
                                               std::to_wstring(int(progress * sourceFilePaths.size())) + L" Files",
                                               int(progress * 100.0f));
...
void QtProjectWizardContentPathsHeaderSearch::finishedAcceptDetectedIncludePathsDialog() {
  const std::vector<std::wstring> detectedPaths =
      utility::split<std::vector<std::wstring>>(m_filesDialog->getText(), L"\n");
  closedFilesDialog();
...
void QtProjectWizardContentPathsHeaderSearch::showDetectedIncludesResult(
  ...
  const std::set<FilePath> headerSearchPaths =
      utility::toSet(m_settings->makePathsExpandedAndAbsolute(m_list->getPathsAsDisplayed()));
  ...
  if (additionalHeaderSearchPaths.empty()) {
    ...
    connect(m_filesDialog, &QtTextEditDialog::finished, this,
            &QtProjectWizardContentPathsHeaderSearch::finishedAcceptDetectedIncludePathsDialog);
    connect(m_filesDialog, &QtTextEditDialog::canceled, this,
...
void QtProjectWizardContentPathsHeaderSearch::showValidationResult(
  ...
  if (unresolvedIncludes.empty()) {
    ...
    for (const IncludeDirective& unresolvedInclude : unresolvedIncludes) {
      orderedIncludes[unresolvedInclude.getIncludingFile().wstr()].emplace(unresolvedInclude.getLineNumber(),
                                                                           unresolvedInclude.getDirective());
    }
    ...
    m_filesDialog =
        new QtTextEditDialog("Unresolved Include Directives",
