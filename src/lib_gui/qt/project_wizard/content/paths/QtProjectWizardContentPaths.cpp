...
void QtProjectWizardContentPaths::setTitleString(const QString& title) { m_titleString = title; }

void QtProjectWizardContentPaths::setHelpString(const QString& help) { m_helpString = help; }
...
void QtProjectWizardContentPaths::detectionClicked() {
  std::vector<FilePath> paths = m_pathDetector->getPathsForDetector(m_detectorBox->currentText().toStdString());
...
void QtProjectWizardContentPaths::detectedPaths(const std::vector<FilePath>& paths) { m_list->setPaths(paths); }
