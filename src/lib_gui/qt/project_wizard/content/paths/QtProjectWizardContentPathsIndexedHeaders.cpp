    ...
    if (!codeblocksProjectPath.empty() && codeblocksProjectPath.exists()) {
      if (std::shared_ptr<Codeblocks::Project> codeblocksProject = Codeblocks::Project::load(codeblocksProjectPath)) {
        ...
        for (const FilePath& path :
             codeblocksProject->getAllSourceFilePathsCanonical(settings->getSourceExtensions())) {
...
QtProjectWizardContentPathsIndexedHeaders::QtProjectWizardContentPathsIndexedHeaders(
    std::shared_ptr<SourceGroupSettings> settings, QtProjectWizardWindow* window, const std::string& projectKindName)
  ...
  setHelpString(
      QString::fromStdString("The provided " + m_projectKindName +
...
void QtProjectWizardContentPathsIndexedHeaders::buttonClicked() {
  ...
  if (!m_filesDialog) {
    if (std::shared_ptr<SourceGroupSettingsCxxCodeblocks> codeblocksSettings =
      ...
      m_filesDialog =
          new QtSelectPathsDialog("Select from Include Paths",
      ...
      connect(m_filesDialog, &QtSelectPathsDialog::finished, this,
              &QtProjectWizardContentPathsIndexedHeaders::savedFilesDialog);
      connect(m_filesDialog, &QtSelectPathsDialog::canceled, this,
      ...
      dynamic_cast<QtSelectPathsDialog*>(m_filesDialog)
          ->setPathsList(utility::convert<FilePath, FilePath>(
                         ...
                         codeblocksSettings->getIndexedHeaderPaths(), m_settings->getProjectDirectoryPath());
    } else if (std::shared_ptr<SourceGroupSettingsCxxCdb> cdbSettings =
      ...
      dynamic_cast<QtSelectPathsDialog*>(m_filesDialog)
          ->setPathsList(utility::convert<FilePath, FilePath>(
                             ...
                             [&](const FilePath& path) { return utility::getAsRelativeIfShorter(path, projectPath); }),
                         cdbSettings->getIndexedHeaderPaths(), m_settings->getProjectDirectoryPath());
    }
  }
