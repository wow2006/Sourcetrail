...
bool QtProjectWizardContent::check() { return true; }

std::vector<FilePath> QtProjectWizardContent::getFilePaths() const { return {}; }

QString QtProjectWizardContent::getFileNamesTitle() const { return QStringLiteral("File List"); }

QString QtProjectWizardContent::getFileNamesDescription() const { return QStringLiteral("files"); }

bool QtProjectWizardContent::isRequired() const { return m_isRequired; }

void QtProjectWizardContent::setIsRequired(bool isRequired) { m_isRequired = isRequired; }
...
QtHelpButton* QtProjectWizardContent::addHelpButton(const QString& helpTitle, const QString& helpText,
...
void QtProjectWizardContent::showFilesDialog(const std::vector<FilePath>& filePaths) {
  if (!m_filesDialog) {
    m_filesDialog = new QtTextEditDialog(getFileNamesTitle(),
                                         QString::number(filePaths.size()) + " " + getFileNamesDescription(), m_window);
    m_filesDialog->setup();
