...
void QtProjectWizardContentVS::populate(QGridLayout* layout, int& row) {
  ...
  QLabel* descriptionLabel = createFormSubLabel(QString::fromStdString(
      ...
      "installed "
      "<a href=\"" +
      utility::getDocumentationLink() + "#visual-studio\">Sourcetrail  Visual Studio  Extension</a>)."));
  ...
  QLabel* skipLabel =
      createFormLabel(QStringLiteral("*Skip this step if you already have a Compilation Database for your Solution."));
...
void QtProjectWizardContentVS::handleVSCDBClicked() { MessageIDECreateCDB().dispatch(); }
