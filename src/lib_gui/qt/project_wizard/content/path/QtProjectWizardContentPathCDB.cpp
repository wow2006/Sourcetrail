...
QtProjectWizardContentPathCDB::QtProjectWizardContentPathCDB(std::shared_ptr<SourceGroupSettingsCxxCdb> settings,
                                                             QtProjectWizardWindow* window)
    : QtProjectWizardContentPath(window), m_settings(settings), m_filePaths([&]() {
        return utility::getAsRelativeIfShorter(utility::toVector(SourceGroupCxxCdb(m_settings).getAllSourceFilePaths()),
...
std::vector<FilePath> QtProjectWizardContentPathCDB::getFilePaths() const { return m_filePaths.getValue(); }

QString QtProjectWizardContentPathCDB::getFileNamesTitle() const { return QStringLiteral("Source Files"); }
...
void QtProjectWizardContentPathCDB::onPickerTextChanged(const QString& text) {
  const FilePath cdbPath =
      utility::getExpandedAndAbsolutePath(FilePath(text.toStdWString()), m_settings->getProjectDirectoryPath());
...
std::shared_ptr<SourceGroupSettings> QtProjectWizardContentPathCDB::getSourceGroupSettings() { return m_settings; }
