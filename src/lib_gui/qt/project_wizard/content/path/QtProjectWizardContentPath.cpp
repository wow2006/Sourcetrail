...
bool QtProjectWizardContentPath::check() {
  ...
  while (true) {
    ...
    FilePath path = utility::getExpandedAndAbsolutePath(FilePath(m_picker->getText().toStdWString()),
...
void QtProjectWizardContentPath::setTitleString(const QString& title) { m_titleString = title; }

void QtProjectWizardContentPath::setHelpString(const QString& help) { m_helpString = help; }

void QtProjectWizardContentPath::setPlaceholderString(const QString& placeholder) { m_placeholderString = placeholder; }
