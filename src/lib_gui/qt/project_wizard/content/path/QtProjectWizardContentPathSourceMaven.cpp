...
std::vector<FilePath> QtProjectWizardContentPathSourceMaven::getFilePaths() const {
  {
    ...
    const std::wstring errorMessage = utility::mavenGenerateSources(mavenPath, mavenSettingsPath, mavenProjectRoot);
  ...
  return utility::getAsRelativeIfShorter(utility::toVector(SourceGroupJavaMaven(m_settings).getAllSourceFilePaths()),
