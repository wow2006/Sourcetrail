...
QtProjectWizardContentFlags::QtProjectWizardContentFlags(
    std::shared_ptr<SourceGroupSettingsWithCxxPathsAndFlags> settings, QtProjectWizardWindow* window,
    bool indicateAsAdditional)
    : QtProjectWizardContent(window), m_settings(settings), m_indicateAsAdditional(indicateAsAdditional) {}
...
void QtProjectWizardContentFlags::populate(QGridLayout* layout, int& row) {
  ...
  addHelpButton(labelText,
                QStringLiteral("<p>Define additional Clang compiler flags used during indexing. Here are some "
                               "examples:</p>"
...
void QtProjectWizardContentFlags::load() { m_list->setStrings(m_settings->getCompilerFlags()); }

void QtProjectWizardContentFlags::save() { m_settings->setCompilerFlags(m_list->getStrings()); }
