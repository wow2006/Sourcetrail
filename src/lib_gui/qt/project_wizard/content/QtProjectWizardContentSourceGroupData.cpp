...
void QtProjectWizardContentSourceGroupData::populate(QGridLayout* layout, int& row) {
  ...
  layout->addWidget(createFormLabel(QStringLiteral("Source Group Name")),
                    row,
  ...
  layout->addWidget(createFormSubLabel(QStringLiteral("Status")),
                    row,
                    QtProjectWizardWindow::FRONT_COL,
                    Qt::AlignRight);
...
void QtProjectWizardContentSourceGroupData::save() {
  ...
  m_settings->setStatus(m_status->isChecked() ? SOURCE_GROUP_STATUS_ENABLED :
...
void QtProjectWizardContentSourceGroupData::changedStatus(bool /*checked*/) {
  emit statusUpdated(m_status->isChecked() ? SOURCE_GROUP_STATUS_ENABLED :
                                             SOURCE_GROUP_STATUS_DISABLED);
