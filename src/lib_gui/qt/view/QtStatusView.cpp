...
QtStatusView::QtStatusView(ViewLayout* viewLayout): StatusView(viewLayout) {
  ...
  const utility::StatusFilter filter = ApplicationSettings::getInstance()->getStatusFilter();
  m_showInfo = createFilterCheckbox(
      QStringLiteral("Info"), filters, filter & utility::StatusType::STATUS_INFO);
  m_showErrors = createFilterCheckbox(
      QStringLiteral("Error"), filters, filter & utility::StatusType::STATUS_ERROR);
...
void QtStatusView::addStatus(const std::vector<utility::Status>& status) {
  m_onQtThread([=]() {
    for(const auto& s : status) {
      ...
      QString statusType = (s.type == utility::StatusType::STATUS_ERROR ? QStringLiteral("ERROR") :
                                                                          QStringLiteral("INFO"));
      ...
      if(s.type == utility::StatusType::STATUS_ERROR) {
...
QCheckBox* QtStatusView::createFilterCheckbox(const QString& name, QBoxLayout* layout, bool checked) {
  ...
  connect(checkbox, &QCheckBox::stateChanged, [=](int) {
    ...
    const auto statusMask = (m_showInfo->isChecked() ? utility::StatusType::STATUS_INFO : 0) +
        (m_showErrors->isChecked() ? utility::StatusType::STATUS_ERROR : 0);
