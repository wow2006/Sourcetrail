...
void QtCodeView::createWidgetWrapper() { setWidgetWrapper(std::make_shared<QtViewWidgetWrapper>(m_widget)); }
...
bool QtCodeView::isVisible() const { return m_widget->isVisible(); }
...
bool QtCodeView::showsErrors() const { return m_widget->hasErrors(); }

void QtCodeView::showSnippets(const std::vector<CodeFileParams>& files, const CodeParams& params,
...
void QtCodeView::showSingleFile(const CodeFileParams& file, const CodeParams& params,
                                const CodeScrollParams& scrollParams) {
  m_onQtThread([=]() {
...
bool QtCodeView::isInListMode() const { return m_widget->isInListMode(); }

void QtCodeView::setMode(bool listMode) {
  m_widget->setMode(listMode ? QtCodeNavigator::MODE_LIST : QtCodeNavigator::MODE_SINGLE);
}

bool QtCodeView::hasSingleFileCached(const FilePath& filePath) const { return m_widget->hasSingleFileCached(filePath); }
...
bool QtCodeView::hasNavigationFocus() const { return m_hasFocus; }
...
void QtCodeView::setNavigationState(const CodeParams& params) {
  ...
  if (params.activeLocalSymbolIds.size()) {
    if (params.activeLocalSymbolType == LOCATION_TOKEN) {
      m_widget->setCurrentActiveTokenIds(params.currentActiveLocalLocationIds.size() ? std::vector<Id>()
  ...
  m_widget->updateReferenceCount(params.referenceCount, params.referenceIndex, params.localReferenceCount,
