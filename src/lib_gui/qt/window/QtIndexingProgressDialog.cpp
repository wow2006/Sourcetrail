...
QSize QtIndexingProgressDialog::sizeHint() const { return QSize(350, 350); }

void QtIndexingProgressDialog::updateIndexingProgress(size_t fileCount, size_t totalFileCount,
...
void QtIndexingProgressDialog::onHidePressed() { emit visibleChanged(false); }
...
void QtIndexingProgressDialog::closeEvent(QCloseEvent* event) { emit visibleChanged(false); }
...
void QtIndexingProgressDialog::setGeometries() {
  ...
  if (m_filePathLabel) {
    m_filePathLabel->setText(
        m_filePathLabel->fontMetrics().elidedText(m_sourcePath, Qt::ElideLeft, m_filePathLabel->width()));
  }
}
