...
QtLicenseWindow::QtLicenseWindow(QWidget* parent) : QtWindow(false, parent) { setScrollAble(true); }

QSize QtLicenseWindow::sizeHint() const { return QSize(650, 600); }

void QtLicenseWindow::populateWindow(QWidget* widget) {
  QVBoxLayout* layout = new QVBoxLayout(widget);

  QLabel* licenseName = new QLabel();
  licenseName->setText(QString::fromLatin1(licenseApp.name) +
  ...
  for (LicenseInfo license : licenses3rdParties) {
    ...
    QLabel* licenseURL = new QLabel();
    licenseURL->setText(QString::fromLatin1("<a href=\"%1\">%1</a>").arg(QString::fromLatin1(license.url)));
    licenseURL->setOpenExternalLinks(true);
    layout->addWidget(licenseURL);
