...
MouseReleaseFilter::MouseReleaseFilter(QObject* parent) : QObject(parent) {
  m_backButton = static_cast<size_t>(ApplicationSettings::getInstance()->getControlsMouseBackButton());
  m_forwardButton = static_cast<size_t>(ApplicationSettings::getInstance()->getControlsMouseForwardButton());
}
...
void QtMainWindow::removeView(View* view) {
  for (size_t i = 0; i < m_dockWidgets.size(); i++) {
    if (m_dockWidgets[i].view == view) {
      ...
      m_dockWidgets.erase(m_dockWidgets.begin() + static_cast<long>(i));
      return;
    }
  }
}
...
void QtMainWindow::showEvent([[maybe_unused]] QShowEvent* e) { m_windowsTaskbarButton.setWindow(this); }
...
void QtMainWindow::closeEvent([[maybe_unused]] QCloseEvent* event) { MessageWindowClosed().dispatchImmediately(); }
...
bool QtMainWindow::focusNextPrevChild([[maybe_unused]] bool next) {
...
void QtMainWindow::openHistoryAction() {
  QAction* action = qobject_cast<QAction*>(sender());
  if (action) {
    std::shared_ptr<MessageBase> m = m_history[static_cast<size_t>(action->data().toInt())];
    m->setSchedulerId(TabId::currentTab());
...
void QtMainWindow::activateBookmarkAction() {
  QAction* action = qobject_cast<QAction*>(sender());
  if (action) {
    std::shared_ptr<Bookmark> bookmark = m_bookmarks[static_cast<size_t>(action->data().toInt())];
