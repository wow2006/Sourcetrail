...
size_t QtProgressBarDialog::getProgress() const { return m_progressBar->getProgress(); }
...
void QtProgressBarDialog::setGeometries() {
  if (m_top) {
    ...
    m_top->setGeometry(0, 0, m_window->size().width(), static_cast<int>(m_window->size().height() * m_topRatio));

    m_title->setGeometry(45, static_cast<int>(m_window->size().height() * m_topRatio - 50),
                         m_window->size().width() - 60, 40);
  }

  if (m_progressBar) {
    m_progressBar->setGeometry(0, static_cast<int>(m_window->size().height() * m_topRatio - 5),
                               m_window->size().width(), 10);
  }
}
