...
QtBookmarkCreator::QtBookmarkCreator(ControllerProxy<BookmarkController>* controllerProxy, QWidget* parent,
                                     Id bookmarkId)
...
void QtBookmarkCreator::setupBookmarkCreator() {
  ...
  {
    // title
    QLabel* title = new QLabel(m_editBookmarkId ? QStringLiteral("Edit Bookmark") : QStringLiteral("Create Bookmark"));
...
void QtBookmarkCreator::setNodeId(Id nodeId) { m_nodeId = nodeId; }
...
void QtBookmarkCreator::handleNext() {
  ...
  if (m_editBookmarkId) {
    m_controllerProxy->executeAsTaskWithArgs(&BookmarkController::editBookmark, m_editBookmarkId, name, comment,
                                             category);
  } else {
    m_controllerProxy->executeAsTaskWithArgs(&BookmarkController::createBookmark, name, comment, category, m_nodeId);
...
void QtBookmarkCreator::handleClose() { close(); }

void QtBookmarkCreator::onNameChanged(const QString& text) { setNextEnabled(text.length() > 0); }
