...
QtIndexingReportDialog::QtIndexingReportDialog(size_t indexedFileCount, size_t totalIndexedFileCount,
                                               size_t completedFileCount, size_t totalFileCount, float time,
                                               bool interrupted, bool shallow, QWidget* parent)
  ...
  QtIndexingDialog::createMessageLabel(m_layout)->setText(QStringLiteral("Total files completed:   ") +
                                                          QString::number(completedFileCount) + "/" +
                                                          QString::number(totalFileCount));

  m_layout->addSpacing(12);
  QtIndexingDialog::createMessageLabel(m_layout)->setText(QStringLiteral("Time:   ") +
                                                          QString::fromStdString(TimeStamp::secondsToString(time)));
...

  {
    ...
    QPushButton* confirmButton = new QPushButton(
        interrupted ? QStringLiteral("Keep") : (shallow ? QStringLiteral("Later") : QStringLiteral("OK")));
...
QSize QtIndexingReportDialog::sizeHint() const { return QSize(m_interrupted ? 400 : 430, 280); }
...
void QtIndexingReportDialog::closeEvent(QCloseEvent* event) { emit QtIndexingDialog::canceled(); }
...
void QtIndexingReportDialog::onDiscardPressed() { emit QtIndexingDialog::canceled(); }

void QtIndexingReportDialog::onStartInDepthPressed() { emit requestReindexing(); }
