...
QtHighlighter::HighlightType QtHighlighter::highlightTypeFromString(const std::string& typeStr) {
  const std::array<HighlightType, 8> types = {
      HighlightType::COMMENT, HighlightType::DIRECTIVE, HighlightType::FUNCTION, HighlightType::KEYWORD,
      HighlightType::NUMBER,  HighlightType::QUOTATION, HighlightType::TEXT,     HighlightType::TYPE};
...
void QtHighlighter::loadHighlightingRules() {
  ...
  for (const FilePath& path :
    ...
    for (QJsonValueRef value : doc.array()) {
      ...
      HighlightType type = highlightTypeFromString(ruleObj.value(QStringLiteral("type")).toString().toStdString());
...
void QtHighlighter::clearHighlightingRules() { s_highlightingRules.clear(); }

QtHighlighter::QtHighlighter(QTextDocument* document, const std::wstring& language) : m_document(document) {
...
std::vector<std::tuple<QtHighlighter::HighlightType, int, int>> QtHighlighter::createMultiLineRangesForRules(
    QTextDocument* doc, std::vector<std::tuple<HighlightType, int, int>>* ranges, const HighlightingRule* startRule,
...
QtHighlighter::HighlightingRule::HighlightingRule(HighlightType type, const QRegExp& regExp, bool priority,
                                                  bool multiLine)
...
void QtHighlighter::formatBlockIfInRange(const QTextBlock& block, HighlightType type,
                                         std::vector<std::tuple<HighlightType, int, int>>* ranges) {
...
void QtHighlighter::formatBlockIfInRange(const QTextBlock& block,
                                         std::vector<std::tuple<HighlightType, int, int>>* ranges) {
  int startPos = block.position();
  int endPos = startPos + block.length() - 1;
...
QTextDocument* QtHighlighter::document() const { return m_document; }
