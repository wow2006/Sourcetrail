...
void QtHighlighter::loadHighlightingRules() {
  ...
  for(const auto& path: utility::file::FileSystem::getFilePathsFromDirectory(
    ...
    for(QJsonValueRef value: doc.array()) {
      ...
      if(!range.empty()) {
        rules.push_back(
            HighlightingRule(type, QRegExp(range.value("start").toString()), priority, true));
        rules.push_back(
...
void QtHighlighter::createRanges(QTextDocument* doc,
                                 const std::vector<HighlightingRule>& singleLineRules) {
...
void QtHighlighter::formatBlockForRule(const QTextBlock& block,
