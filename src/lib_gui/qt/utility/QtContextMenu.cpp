...
QtContextMenu::QtContextMenu(QContextMenuEvent* event, QWidget* origin) : m_menu(origin), m_point(event->globalPos()) {
  getInstance();
}

void QtContextMenu::addAction(QAction* action) { m_menu.addAction(action); }

void QtContextMenu::enableUndo(bool enabled) { s_undoAction->setEnabled(enabled); }

void QtContextMenu::enableRedo(bool enabled) { s_redoAction->setEnabled(enabled); }
...
QtContextMenu* QtContextMenu::getInstance() {
  if (!s_instance) {
    ...
    connect(s_openContainingFolderAction, &QAction::triggered, s_instance,
...
void QtContextMenu::addSeparator() { m_menu.addSeparator(); }

void QtContextMenu::show() { m_menu.exec(m_point); }

void QtContextMenu::undoActionTriggered() { MessageHistoryUndo().dispatch(); }

void QtContextMenu::redoActionTriggered() { MessageHistoryRedo().dispatch(); }
