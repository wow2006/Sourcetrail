...
class QtHighlighter {
 public:
  enum class HighlightType { COMMENT, DIRECTIVE, FUNCTION, KEYWORD, NUMBER, QUOTATION, TEXT, TYPE };
 ...
 private:
  ...
  std::vector<std::tuple<HighlightType, int, int>> getRangesForRule(const QTextBlock& block,
                                                                    const HighlightingRule& rule) const;

  void formatBlockForRule(const QTextBlock& block, const HighlightingRule& rule,
                          std::vector<std::tuple<HighlightType, int, int>>* ranges);
  void formatBlockIfInRange(const QTextBlock& block, HighlightType type,
                            std::vector<std::tuple<HighlightType, int, int>>* ranges);
  void formatBlockIfInRange(const QTextBlock& block, std::vector<std::tuple<HighlightType, int, int>>* ranges);
