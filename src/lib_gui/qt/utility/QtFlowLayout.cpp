...
QtFlowLayout::QtFlowLayout(int margin, int hSpacing, int vSpacing) : m_hSpace(hSpacing), m_vSpace(vSpacing) {
  setContentsMargins(margin, margin, margin, margin);
}
...
void QtFlowLayout::addItem(QLayoutItem* item) { itemList.append(item); }
...
int QtFlowLayout::count() const { return itemList.size(); }

QLayoutItem* QtFlowLayout::itemAt(int index) const { return itemList.value(index); }
...
bool QtFlowLayout::hasHeightForWidth() const { return true; }
...
QSize QtFlowLayout::sizeHint() const { return minimumSize(); }
