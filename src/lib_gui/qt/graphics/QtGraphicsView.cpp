...
float QtGraphicsView::getZoomFactor() const { return m_appZoomFactor * m_zoomFactor; }
...
void QtGraphicsView::stopTimer() { m_timer->stop(); }

void QtGraphicsView::openInTab() { MessageTabOpenWith(m_openInTabNodeId).dispatch(); }
...
QImage QtGraphicsView::toQImage() {
  ...
  }
  {
    ...
    painter.drawText(QRect(margin, margin, image.size().width() - 2 * margin, image.size().height() - 2 * margin),
                     Qt::AlignBottom | Qt::AlignHCenter, exportNotice, &boundingRect);
    ...
    painter.drawText(boundingRect.right() + boundingRect.height() / 5, boundingRect.top() + boundingRect.height() / 2,
                     QChar(0x00AE));
  }
...
void QtGraphicsView::exportGraph() {
  ...
  FilePath filePath(
      QtFileDialog::showSaveFileDialog(nullptr, QStringLiteral("Save image"), FilePath(),
  ...
  if (filePath.extension() == L".svg") {
    ...
    }
    {
      ...
      painter.drawText(QRect(margin, margin, svgGen.size().width() - 2 * margin, svgGen.size().height() - 2 * margin),
                       Qt::AlignBottom | Qt::AlignHCenter, exportNotice + ' ' + QChar(0x00AE), &boundingRect);
    }
...
void QtGraphicsView::copyGraph() { QApplication::clipboard()->setImage(toQImage()); }

void QtGraphicsView::copyNodeName() {
  QApplication::clipboard()->setText(QString::fromStdWString(m_clipboardNodeName));
}

void QtGraphicsView::collapseNode() { MessageGraphNodeExpand(m_collapseNodeId, false).dispatch(); }

void QtGraphicsView::expandNode() { MessageGraphNodeExpand(m_expandNodeId, true).dispatch(); }

void QtGraphicsView::showInIDE() { MessageCodeShowDefinition(m_hideNodeId, true).dispatch(); }

void QtGraphicsView::showDefinition() { MessageCodeShowDefinition(m_hideNodeId).dispatch(); }

void QtGraphicsView::hideNode() { MessageGraphNodeHide(m_hideNodeId).dispatch(); }

void QtGraphicsView::hideEdge() { MessageGraphNodeHide(m_hideEdgeId).dispatch(); }

void QtGraphicsView::bookmarkNode() { MessageBookmarkCreate(m_bookmarkNodeId).dispatch(); }

void QtGraphicsView::zoomInPressed() { updateZoom(m_zoomInButtonSpeed); }

void QtGraphicsView::zoomOutPressed() { updateZoom(m_zoomOutButtonSpeed); }

void QtGraphicsView::hideZoomLabel() { m_zoomState->hide(); }

void QtGraphicsView::legendClicked() { MessageActivateLegend().dispatch(); }

bool QtGraphicsView::moves() const { return m_up || m_down || m_left || m_right; }
