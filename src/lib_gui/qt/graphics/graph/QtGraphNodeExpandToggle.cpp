...
bool QtGraphNodeExpandToggle::isExpandToggleNode() const { return true; }
...
void QtGraphNodeExpandToggle::updateStyle() {
  ...
  float textX = static_cast<float>((m_rect->rect().width() / 2) -
                                   (QFontMetrics(m_text->font()).boundingRect(m_text->text()).width() / 2));
  const float textY = static_cast<float>(m_rect->rect().height() / 2 - QFontMetrics(m_text->font()).height() / 1.8f);
  ...
  m_icon->setPos((m_rect->rect().width() - m_icon->pixmap().width() / QtDeviceScaledPixmap::devicePixelRatio()) / 2,
...
bool QtGraphNodeExpandToggle::isExpanded() const { return m_expanded; }
