...
QtGraphEdge::QtGraphEdge(GraphFocusHandler* focusHandler,
                         QtGraphNode* owner,
                         QtGraphNode* target,
                         const Edge* data,
                         size_t weight,
  ...
  if(m_useBezier) {
    ...
    for(const Vec4i& rect: m_path) {
      ...
      if(route == QtLineItemBase::ROUTE_HORIZONTAL) {
        line->updateLine(Vec2i(rect.x(), (rect.y() + rect.w()) / 2),
                         Vec2i(rect.z(), (rect.y() + rect.w()) / 2),
                         style);
      } else {
        line->updateLine(Vec2i((rect.x() + rect.z()) / 2, rect.y()),
    ...
    } else if(type == Edge::EDGE_INHERITANCE ||
    ...
    } else if(type != Edge::EDGE_BUNDLED_EDGES || owner != ownerNonGroupParent ||
              target != targetNonGroupParent) {
...
void QtGraphEdge::onClick() {
  if(isExpandable()) {
    QtGraphNode* node = (m_direction == TokenComponentBundledEdges::DIRECTION_BACKWARD ? m_owner :
                                                                                         m_target);
    ...
    MessageActivateTrailEdge({getData()->getId()},
                             getData()->getType(),
