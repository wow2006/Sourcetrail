  ...
  for (const TooltipSnippet& snippet : info.snippets) {
    ...
    if (size.width() > maxWidth) {
      field->setMinimumSize(
          QSize(maxWidth, size.height() + QApplication::style()->pixelMetric(QStyle::PM_ScrollBarExtent)));
    } else {
...
void QtTooltip::setParentView(QWidget* parentView) { m_parentView = parentView; }

bool QtTooltip::isHovered() const { return m_isHovered; }
...
void QtTooltip::enterEvent(QEvent* event) { m_isHovered = true; }
