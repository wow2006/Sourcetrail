...
void QtSearchElement::onChecked(bool) { emit wasChecked(this); }
...
QCompleter* QtSmartSearchBox::getCompleter() const { return m_completer; }

std::vector<SearchMatch> QtSmartSearchBox::getMatches() const { return utility::toVector(m_matches); }
...
void QtSmartSearchBox::setAutocompletionList(const std::vector<SearchMatch>& autocompletionList) {
  ...
  connect(completer, &QtAutocompletionList::matchHighlighted, this, &QtSmartSearchBox::onAutocompletionHighlighted,
          Qt::DirectConnection);
  connect(completer, &QtAutocompletionList::matchActivated, this, &QtSmartSearchBox::onAutocompletionActivated,
          Qt::DirectConnection);
...
void QtSmartSearchBox::refreshStyle() { updateElements(); }
...
void QtSmartSearchBox::keyReleaseEvent(QKeyEvent* event) { m_shiftKeyDown = event->modifiers() & Qt::ShiftModifier; }
...
void QtSmartSearchBox::onAutocompletionHighlighted(const SearchMatch& match) { m_highlightedMatch = match; }
...
void QtSmartSearchBox::moveCursor(int offset) { moveCursorTo(static_cast<int>(m_cursorIndex + offset)); }
...
void QtSmartSearchBox::hideAutoCompletions() { m_completer->popup()->hide(); }
...
bool QtSmartSearchBox::lastMatchIsNoFilter() const { return m_matches.empty() || !m_matches.back().isFilterCommand(); }
