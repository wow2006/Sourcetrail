...
void QtAutocompletionDelegate::paint(QPainter* painter,
  ...
  if(type.size() && type != QLatin1String("command") && type != QLatin1String("filter")) {
    ...
    fillColor = QColor(scheme
                           ->getSearchTypeColor(utility::encodeToUtf8(SearchMatch::getSearchTypeName(
                                                    SearchMatch::SEARCH_COMMAND)),
                                                "fill")
                           .c_str());
  ...
  painter->drawText(option.rect.adjusted(static_cast<int>(m_charWidth1 + 2), top1 - 3, 0, 0),
                    Qt::AlignLeft,
                    highlightText);
  painter->restore();
  ...
  if(type.size()) {
    ...
    painter->fillRect(QRect(static_cast<int>(x - m_charWidth2),
                            y,
...
QSize QtAutocompletionDelegate::sizeHint(const QStyleOptionViewItem& option,
                                         const QModelIndex& /*index*/) const {
  ...
  return QSize(static_cast<int>(std::max((text.size() + 2) * m_charWidth1,
                                         (subtext.size() + type.size() + 6) * m_charWidth2)),
...
void QtAutocompletionDelegate::calculateCharSizes(QFont font) {
  ...
  m_arrow = QtDeviceScaledPixmap(QString::fromStdString(ResourcePaths::getGuiDirectoryPath().str() +
