...
void QtAutocompletionDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
  ...
  if (type.size() && type != QLatin1String("command") && type != QLatin1String("filter")) {
    const GraphViewStyle::NodeColor& nodeColor =
        GraphViewStyle::getNodeColor(nodeType.getUnderscoredTypeString(), false);
    ...
    fillColor =
        QColor(scheme
                   ->getSearchTypeColor(
                       utility::encodeToUtf8(SearchMatch::getSearchTypeName(SearchMatch::SEARCH_COMMAND)), "fill")
                   .c_str());
    textColor =
        QColor(scheme
                   ->getSearchTypeColor(
                       utility::encodeToUtf8(SearchMatch::getSearchTypeName(SearchMatch::SEARCH_COMMAND)), "text")
                   .c_str());
  }
  ...
  QColor backgroundColor =
      option.palette.color((option.state & QStyle::State_Selected ? QPalette::Highlight : QPalette::Base));
  ...
  if (!indices.empty()) {
    for (int i = 0; i < indices.size(); i++) {
      ...
      QRect rect(static_cast<int>(option.rect.left() + m_charWidth1 * (idx + 1) + 2), option.rect.top() + top1 - 1,
                 static_cast<int>(m_charWidth1 + 1), static_cast<int>(m_charHeight1 - 1));
    ...
    QRect rect(option.rect.left(), option.rect.top() + top1, static_cast<int>(m_charWidth1 - 1),
               static_cast<int>(m_charHeight1 - 2));
    painter->fillRect(rect, fillColor);
  }

  // draw text normal
  painter->drawText(option.rect.adjusted(static_cast<int>(m_charWidth1 + 2), top1 - 3, 0, 0), Qt::AlignLeft, text);
  ...
  painter->drawText(option.rect.adjusted(static_cast<int>(m_charWidth1 + 2), top1 - 3, 0, 0), Qt::AlignLeft,
                    highlightText);
  painter->restore();
  ...
  if (subtext.size()) {
    // draw arrow icon
    painter->drawPixmap(static_cast<int>(option.rect.left() + m_charWidth2 * 2),
    ...
    if (indices.size()) {
      for (int i = 0; i < indices.size(); i++) {
        ...
        QRect rect(static_cast<int>(option.rect.left() + m_charWidth2 * (idx + 3) + 2), option.rect.top() + top2 + 1,
                   static_cast<int>(m_charWidth2 + 1), static_cast<int>(m_charHeight2));
        painter->fillRect(rect, fillColor);
    ...
    // draw subtext normal
    painter->drawText(option.rect.adjusted(static_cast<int>((3 * m_charWidth2) + 2), top2, 0, 0), Qt::AlignLeft,
                      subtext);
  ...
  if (type.size()) {
    ...
    painter->fillRect(QRect(static_cast<int>(x - m_charWidth2), y, static_cast<int>(width + m_charWidth2 * 3),
                            static_cast<int>(m_charHeight2 + 2)),
                      backgroundColor);
    painter->drawText(QRect(x, y, static_cast<int>(width + m_charWidth2), static_cast<int>(m_charHeight2)),
                      Qt::AlignRight, type);
  }
...
QSize QtAutocompletionDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const {
  ...
  return QSize(
      static_cast<int>(std::max((text.size() + 2) * m_charWidth1, (subtext.size() + type.size() + 6) * m_charWidth2)),
...
void QtAutocompletionDelegate::resetCharSizes() { m_charWidth1 = m_charHeight1 = m_charWidth2 = m_charHeight2 = 0.0f; }
...
void QtAutocompletionList::completeAt(QPoint pos, const std::vector<SearchMatch>& autocompletionList) {
  ...
  disconnect();  // must be done because of a bug where signals are no longer received by
                 // QtSmartSearchBox
  connect(this, static_cast<void (QtAutocompletionList::*)(const QModelIndex&)>(&QtAutocompletionList::highlighted),
          this, &QtAutocompletionList::onHighlighted, Qt::DirectConnection);
  connect(this, static_cast<void (QtAutocompletionList::*)(const QModelIndex&)>(&QtAutocompletionList::activated), this,
...
const SearchMatch* QtAutocompletionList::getSearchMatchAt(int idx) const { return m_model->getSearchMatchAt(idx); }
