...
QSize QtSearchBar::sizeHint() const { return QSize(400, 100); }

QString QtSearchBar::query() const { return m_searchBox->text(); }

void QtSearchBar::setMatches(const std::vector<SearchMatch>& matches) { m_searchBox->setMatches(matches); }

void QtSearchBar::setFocus() { m_searchBox->setFocus(); }

void QtSearchBar::findFulltext() { m_searchBox->findFulltext(); }
...
QAbstractItemView* QtSearchBar::getCompleterPopup() { return m_searchBox->getCompleter()->popup(); }

void QtSearchBar::refreshStyle() { m_searchBox->refreshStyle(); }

void QtSearchBar::homeButtonClicked() { MessageActivateOverview().dispatch(); }
