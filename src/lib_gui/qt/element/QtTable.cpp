...
QWidget* SelectableCellDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& option,
...
void QtTable::updateRows() {
  ...
  const int width =
      static_cast<int>(ApplicationSettings::getInstance()->getFontSize() * 0.7 * int(1 + std::log10(rowCount)));
...
void QtTable::showFirstRow() { verticalScrollBar()->setValue(verticalScrollBar()->minimum()); }
...
bool QtTable::hasSelection() const { return this->selectionModel()->hasSelection(); }

void QtTable::columnResized(int column, int oldWidth, int newWidth) { resizeRowsToContents(); }
