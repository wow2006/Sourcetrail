...
QtListBox::QtListBox(QWidget* parent, const QString& listName) : QFrame(parent), m_listName(listName) {
  ...
  QPushButton* editButton =
      new QtIconButton(ResourcePaths::getGuiDirectoryPath().concatenate(L"code_view/images/edit.png"), FilePath());
...
void QtListBox::clear() { m_list->clear(); }
...
void QtListBox::showEditDialog() {
  if (!m_editDialog) {
    m_editDialog =
        std::make_shared<QtTextEditDialog>(m_listName, "Edit the list in plain text. Each line is one item.");
    m_editDialog->setup();
