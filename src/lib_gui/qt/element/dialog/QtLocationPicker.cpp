...
void QtLocationPicker::setPlaceholderText(const QString& text) { m_data->setPlaceholderText(text); }

QString QtLocationPicker::getText() { return m_data->text(); }

void QtLocationPicker::setText(const QString& text) { m_data->setText(text); }

void QtLocationPicker::clearText() { m_data->clear(); }

bool QtLocationPicker::pickDirectory() const { return m_pickDirectory; }

void QtLocationPicker::setPickDirectory(bool pickDirectory) { m_pickDirectory = pickDirectory; }

void QtLocationPicker::setFileFilter(const QString& fileFilter) { m_fileFilter = fileFilter; }

void QtLocationPicker::setRelativeRootDirectory(const FilePath& dir) { m_relativeRootDirectory = dir; }
...
void QtLocationPicker::onHandleButtonPressed() {
  ...
  if (!fileName.isEmpty()) {
    m_data->setText(QString::fromStdWString(
        utility::getAsRelativeIfShorter(FilePath(fileName.toStdWString()), m_relativeRootDirectory).wstr()));
    emit locationPicked();
  }
}

void QtLocationPicker::onDataTextChanged(const QString& text) { emit textChanged(text); }
