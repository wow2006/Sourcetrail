...
QtStatusBar::QtStatusBar() : m_text(this), m_ideStatusText(this) {
  ...
  m_movie = std::make_shared<QMovie>(
      QString::fromStdWString(ResourcePaths::getGuiDirectoryPath().concatenate(L"statusbar_view/loader.gif").wstr()));
...
void QtStatusBar::setText(const std::wstring& text, bool isError, bool showLoader) {
  ...
  m_textString = text;
  m_text.setText(
      m_text.fontMetrics().elidedText(QString::fromStdWString(m_textString), Qt::ElideRight, m_text.width()));
}
...
void QtStatusBar::setIdeStatus(const std::wstring& text) { m_ideStatusText.setText(QString::fromStdWString(text)); }
...
void QtStatusBar::showStatus() { MessageShowStatus().dispatch(); }

void QtStatusBar::showErrors() { MessageErrorsAll().dispatch(); }

void QtStatusBar::clickedIndexingProgress() { MessageIndexingShowDialog().dispatch(); }
