...
QtHistoryItem::QtHistoryItem(const SearchMatch& match, size_t index, bool isCurrent) : index(index), m_match(match) {
  ...
  m_name->setObjectName(isCurrent ? QStringLiteral("history_item_current") : QStringLiteral("history_item"));
  ...
  if (match.searchType == SearchMatch::SEARCH_TOKEN) {
    ...
    m_indicatorHoverColor =
        scheme->getSearchTypeColor(utility::encodeToUtf8(match.getSearchTypeName()), "fill", "hover");
  }
...
QSize QtHistoryItem::getSizeHint() const {
  return QSize(m_name->fontMetrics().boundingRect(m_name->text()).width() + 40, m_name->fontMetrics().height() + 8);
}

const SearchMatch& QtHistoryItem::getMatch() const { return m_match; }
...
void QtHistoryList::closeEvent(QCloseEvent* event) { emit closed(); }
