...
class QtCodeField : public QPlainTextEdit {
 ...
 public:
  ...
  QtCodeField(size_t startLineNumber, const std::string& code, std::shared_ptr<SourceLocationFile> locationFile,
              bool convertLocationsOnDemand = true, QWidget* parent = nullptr);
 ...
 protected:
  ...
  bool annotateText(const std::set<Id>& activeSymbolIds, const std::set<Id>& activeLocationIds,
                    const std::set<Id>& focusedSymbolIds, Id focusedLocationId);
