...
const FilePath& QtCodeFile::getFilePath() const { return m_filePath; }

const QtCodeFileTitleBar* QtCodeFile::getTitleBar() const { return m_titleBar; }
...
const std::vector<QtCodeSnippet*>& QtCodeFile::getSnippets() const { return m_snippets; }
...
void QtCodeFile::requestWholeFileContent(size_t targetLineNumber) {
  if (!m_isWholeFile) {
    MessageChangeFileView msg(
        m_filePath, MessageChangeFileView::FILE_MAXIMIZED, MessageChangeFileView::VIEW_LIST,
...
void QtCodeFile::setIsComplete(bool isComplete) { m_titleBar->setIsComplete(isComplete); }

void QtCodeFile::setIsIndexed(bool isIndexed) { m_titleBar->setIsIndexed(isIndexed); }

bool QtCodeFile::isCollapsed() const { return m_titleBar->isCollapsed(); }
...
bool QtCodeFile::hasSnippets() const { return m_snippets.size() > 0; }
...
void QtCodeFile::updateTitleBar() { m_titleBar->getTitleButton()->updateTexts(); }
...
void QtCodeFile::focusTop() { m_navigator->setFocusedFile(this); }
...
void QtCodeFile::clickedMinimizeButton() {
  MessageChangeFileView msg(m_filePath, MessageChangeFileView::FILE_MINIMIZED, MessageChangeFileView::VIEW_LIST,
...
void QtCodeFile::clickedSnippetButton() {
  MessageChangeFileView msg(m_filePath, MessageChangeFileView::FILE_SNIPPETS, MessageChangeFileView::VIEW_LIST,
                            CodeScrollParams::toFile(m_filePath, CodeScrollParams::Target::VISIBLE));
  msg.setSchedulerId(m_navigator->getSchedulerId());
  msg.dispatch();
}
...
void QtCodeFile::clickedMaximizeButton() {
  ...
  CodeScrollParams scrollParams =
      locationId ? CodeScrollParams::toReference(m_filePath, locationId, 0, CodeScrollParams::Target::CENTER)
                 : CodeScrollParams::toLine(m_filePath, lineNumber, CodeScrollParams::Target::CENTER);

  MessageChangeFileView msg(m_filePath, MessageChangeFileView::FILE_MAXIMIZED, MessageChangeFileView::VIEW_SINGLE,
                            scrollParams, true);
  msg.setSchedulerId(m_navigator->getSchedulerId());
  msg.dispatch();
}
