...
QtCodeSnippet::QtCodeSnippet(const CodeSnippetParams& params, QtCodeNavigator* navigator, QtCodeFile* file)
  ...
  m_codeArea =
      new QtCodeArea(params.startLineNumber, params.code, params.locationFile, navigator, !params.isOverview, this);
...
QtCodeFile* QtCodeSnippet::getFile() const { return m_file; }

QtCodeArea* QtCodeSnippet::getArea() const { return m_codeArea; }

size_t QtCodeSnippet::getStartLineNumber() const { return m_codeArea->getStartLineNumber(); }

size_t QtCodeSnippet::getEndLineNumber() const { return m_codeArea->getEndLineNumber(); }

int QtCodeSnippet::lineNumberDigits() const { return m_codeArea->lineNumberDigits(); }
...
void QtCodeSnippet::setIsActiveFile(bool isActiveFile) { m_codeArea->setIsActiveFile(isActiveFile); }
...
std::string QtCodeSnippet::getCode() const { return m_codeArea->getCode(); }

void QtCodeSnippet::findScreenMatches(const std::wstring& query,
                                      std::vector<std::pair<QtCodeArea*, Id>>* screenMatches) {
  m_codeArea->findScreenMatches(query, screenMatches);
}

bool QtCodeSnippet::hasFocus(const CodeFocusHandler::Focus& focus) const { return m_codeArea == focus.area; }

bool QtCodeSnippet::setFocus(Id locationId) { return m_codeArea->setFocus(locationId); }
...
void QtCodeSnippet::copySelection() { m_codeArea->copySelection(); }
