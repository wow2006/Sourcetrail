...
QtCodeField::QtCodeField(size_t startLineNumber, const std::string& code,
                         std::shared_ptr<SourceLocationFile> locationFile, bool convertLocationsOnDemand,
                         QWidget* parent)
...
QSize QtCodeField::sizeHint() const {
  ...
  for (QTextBlock block = document()->firstBlock(); block.isValid(); block = block.next()) {
    ...
    const int blockWidth = fm.boundingRect(0, 0, 1000000, 1000000, Qt::AlignLeft | Qt::AlignTop | Qt::TextExpandTabs,
                                           block.text(), static_cast<int>(tabStopDistance()))
                               .width();
...
size_t QtCodeField::getStartLineNumber() const { return m_startLineNumber; }

size_t QtCodeField::getEndLineNumber() const { return m_startLineNumber + blockCount() - 1; }

int QtCodeField::totalLineHeight() const {
  return static_cast<int>(blockBoundingRect(firstVisibleBlock()).height() * blockCount());
}

std::string QtCodeField::getCode() const { return m_code; }

std::shared_ptr<SourceLocationFile> QtCodeField::getSourceLocationFile() const { return m_locationFile; }

FilePath QtCodeField::getFilePath() const { return m_locationFile ? m_locationFile->getFilePath() : FilePath(); }

void QtCodeField::annotateText() { annotateText(std::set<Id>(), std::set<Id>(), std::set<Id>(), 0); }
...
void QtCodeField::paintEvent(QPaintEvent* event) {
  ...
  for (const Annotation& annotation : m_annotations) {
    ...
    if (annotation.locationType == LOCATION_SCOPE) {
      painter.drawRoundedRect(0, static_cast<int>(top + (annotation.startLine - m_startLineNumber) * blockHeight),
                              width(), (annotation.endLine - annotation.startLine + 1) * blockHeight, borderRadius,
                              borderRadius);
    } else {
...
void QtCodeField::leaveEvent(QEvent* event) { setHoveredAnnotations(std::vector<const Annotation*>()); }
...
bool QtCodeField::annotateText(const std::set<Id>& activeSymbolIds, const std::set<Id>& activeLocationIds,
                               const std::set<Id>& coFocusedSymbolIds, Id focusedLocationId) {
  for (size_t i = 0; i < m_annotations.size(); i++) {
    ...
    annotation.isActive = (utility::shareElement(activeSymbolIds, annotation.tokenIds) ||
...
void QtCodeField::createAnnotations(std::shared_ptr<SourceLocationFile> locationFile) {
  ...
  locationFile->forEachSourceLocation([&](const SourceLocation* location) {
    ...
    if (!startLocation || startLocation->getLineNumber() < m_startLineNumber) {
      ...
      const int startCol =
          getColumnCorrectedForMultibyteCharacters(startLine, static_cast<int>(startLocation->getColumnNumber() - 1));
    ...
    if (!endLocation || endLocation->getLineNumber() > endLineNumber) {
      ...
      const int endCol =
          getColumnCorrectedForMultibyteCharacters(endLine, static_cast<int>(endLocation->getColumnNumber()));
...
void QtCodeField::activateAnnotations(const std::vector<const Annotation*>& annotations, bool fromMouse,
                                      int mouseOffsetX) {
...
int QtCodeField::startTextEditPosition() const { return 0; }

int QtCodeField::endTextEditPosition() const { return m_endTextEditPosition; }
...
std::vector<QRect> QtCodeField::getCursorRectsForAnnotation(const Annotation& annotation) const {
  ...
  while (line <= annotation.endLine) {
    ...
    rects.push_back(QRect(rectStart.left(), rectStart.top(), rectEnd.right() - rectStart.left(),
