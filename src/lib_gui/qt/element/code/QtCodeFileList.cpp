...
void QtCodeFileList::scrollTo(const utility::file::FilePath& filePath,
  ...
  if(focusTarget) {
    m_navigator->setFocusedLocationId(snippet->getArea(),
                                      lineNumber,
...
void QtCodeFileList::moveFocus(const CodeFocusHandler::Focus& focus,
                               CodeFocusHandler::Direction direction) {
...
void QtCodeFileList::updateFirstSnippetTitleBar(QtCodeFile* file, int fileTitleBarOffset) {
  ...
  if(m_mirroredTitleBar != mirroredTitleBar) {
    ...
    if(m_mirroredTitleBar && file) {
      ...
      connect(m_firstSnippetTitleBar,
      ...
      m_firstSnippetTitleBar->setGeometry(file->pos().x() + mirroredTitleBar->pos().x(),
                                          0,
