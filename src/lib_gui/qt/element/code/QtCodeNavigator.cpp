...
QtCodeNavigator::QtCodeNavigator(QWidget* parent)
...

  {
...

    {
      ...
      connect(m_prevLocalReferenceButton,
              &QPushButton::clicked,
              this,
              &QtCodeNavigator::previousLocalReference);
      connect(m_nextLocalReferenceButton,
              &QPushButton::clicked,
              this,
...
void QtCodeNavigator::updateReferenceCount(size_t referenceCount,
                                           size_t referenceIndex,
                                           size_t localReferenceCount,
                                           size_t localReferenceIndex) {
  if(referenceIndex != referenceCount) {
    m_refLabel->setText(QString::number(referenceIndex + 1) + "/" +
                        QString::number(referenceCount) + " references");
  } else {
  ...
  if(localReferenceIndex != localReferenceCount) {
    m_localRefLabel->setText(QString::number(localReferenceIndex + 1) + "/" +
                             QString::number(localReferenceCount) + " local references");
  } else {
  ...
  m_localRefLabel->setMinimumWidth(
      m_localRefLabel->fontMetrics()
          .boundingRect(QString(QString::number(localReferenceCount).size() * 2, 'a') +
                        "/ local references")
          .width() +
      30);
...
void QtCodeNavigator::setCurrentActiveLocalLocationIds(
    const std::vector<Id>& currentActiveLocalLocationIds) {
...
void QtCodeNavigator::setActiveLocalTokenIds(const std::vector<Id>& activeLocalTokenIds,
...
void QtCodeNavigator::scrollTo(const CodeScrollParams& params, bool animated, bool focusTarget) {
  ...
  case CodeScrollParams::Type::TO_REFERENCE:
    func = [=]() {
      m_current->scrollTo(params.filePath,
                          0,
                          params.locationId,
...
void QtCodeNavigator::scrollToFocus() {
  ...
  if(focus.file) {
    scrollTo(CodeScrollParams::toFile(focus.file->getFilePath(), CodeScrollParams::Target::VISIBLE),
             true,
             false);
  } else if(focus.scopeLine) {
    scrollTo(CodeScrollParams::toLine(
