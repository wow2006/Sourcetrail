...
void TaskExecuteCustomCommands::runPythonPostProcessing(PersistentStorage& storage) {
  ...
  for (const StorageSourceLocation location : storage.getStorageSourceLocations()) {
    if (intToLocationType(location.type) == LOCATION_UNSOLVED)  // FIXME: this doesn't catch unsolved qualifiers ->
                                                                // convert Qualifier location type to qualifier edge
    {
  ...
  locationCollection->forEachSourceLocationFile([&nodeNameToStorageNodes, &storage, &dataToInsert,
                                                 &occurrencesToDelete](
    ...
    if (textAccess) {
      ...
      locationFile->forEachStartSourceLocation([textAccess, &childToParentNodesMap, &nodeNameToStorageNodes, &storage,
                                                &dataToInsert, &occurrencesToDelete](const SourceLocation* startLoc) {
        if (!startLoc) {
          return;
        }
        ...
        const std::wstring token = utility::decodeFromUtf8(tokenLine.substr(
            startLoc->getColumnNumber() - 1, endLoc->getColumnNumber() - startLoc->getColumnNumber() + 1));
        ...
        }
        {
          ...
          if (!matches.empty()) {
            for (const Id elementId : startLoc->getTokenIds()) {
              ...
              if (edge.id != 0) {
                NameHierarchy nameHierarchy = storage.getNameHierarchyForNodeId(edge.sourceNodeId);
                if (nameHierarchy.size() > 1) {
                  std::wstring name = nameHierarchy.getRange(0, nameHierarchy.size() - 1).back().getName();
        ...
        }
        {
          ...
          if (!definitionContextName.empty()) {
            for (const StorageNode& node : nodeNameToStorageNodes[token]) {
              ...
              if (nameHierarchy.size() > 1 &&
                  nameHierarchy.getRange(0, nameHierarchy.size() - 1).back().getName() == definitionContextName) {
              ...
              if (edge.id != 0)  // for node elements this condition will fail
              {
                if (Edge::intToType(edge.type) == Edge::EDGE_INHERITANCE) {
                  if (intToNodeKind(targetNode.type) == NODE_CLASS) {
                    const NameHierarchy chileName = storage.getNameHierarchyForNodeId(edge.sourceNodeId);
                    const NameHierarchy parentName = NameHierarchy::deserialize(targetNode.serializedName);
                    childToParentNodesMap[chileName.back().getName()].push_back(parentName.back().getName());
                  } else {
                    continue;
                  }
                }
                ...
                dataToInsert.push_back(
                    {StorageEdgeData(edge.type, edge.sourceNodeId, targetNode.id), startLoc->getLocationId()});
  ...
  if (ambiguousEdgeIds.size() == dataToInsert.size()) {
    for (size_t i = 0; i < ambiguousEdgeIds.size(); i++) {
      storage.addElementComponent(StorageElementComponent(
          ambiguousEdgeIds[i], elementComponentKindToInt(ElementComponentKind::IS_AMBIGUOUS), L""));
...
TaskExecuteCustomCommands::TaskExecuteCustomCommands(std::unique_ptr<IndexerCommandProvider> indexerCommandProvider,
                                                     ...
                                                     std::shared_ptr<DialogView> dialogView, size_t indexerThreadCount,
...
Task::TaskState TaskExecuteCustomCommands::doUpdate(std::shared_ptr<Blackboard> blackboard) {
  ...
  for (size_t i = 1 /*this method is counting as the first thread*/; i < m_indexerThreadCount; i++) {
    indexerThreads.push_back(std::make_shared<std::thread>(&TaskExecuteCustomCommands::executeParallelIndexerCommands,
                                                           this, static_cast<int>(i), blackboard));
  }
...
void TaskExecuteCustomCommands::runIndexerCommand(std::shared_ptr<IndexerCommandCustom> indexerCommand,
  ...
  if (indexerCommand) {
    ...
    m_dialogView->updateCustomIndexingDialog(indexedSourceFileCount + 1, indexedSourceFileCount, m_indexerCommandCount,
                                             {sourcePath});
    ...
    if (storage) {
      ...
      if (currentErrorCount.m_total > previousErrorCount.m_total) {
        const ErrorCountInfo diff(currentErrorCount.m_total - previousErrorCount.m_total,
    ...
    if (out.exitCode == 0 && out.error.empty()) {
      ...
      std::wstring statusText =
          L"command \"" + indexerCommand->getCommand() + L" " + utility::join(arguments, L" ") + L"\" returned";
      if (out.exitCode != 0) {
