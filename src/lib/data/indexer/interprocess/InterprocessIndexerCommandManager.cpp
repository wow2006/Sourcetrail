...
InterprocessIndexerCommandManager::InterprocessIndexerCommandManager(const std::string& instanceUuid, Id processId,
                                                                     bool isOwner)
    : BaseInterprocessDataManager(s_sharedMemoryNamePrefix.data() + instanceUuid, 1048576 /* 1 MB */, instanceUuid,
                                  processId, isOwner) {}
...
void InterprocessIndexerCommandManager::pushIndexerCommands(
  ...
  SharedMemory::Queue<SharedIndexerCommand>* queue =
      access.accessValueWithAllocator<SharedMemory::Queue<SharedIndexerCommand>>(s_indexerCommandsKeyName.data());
  if (!queue) {
    return;
  }
