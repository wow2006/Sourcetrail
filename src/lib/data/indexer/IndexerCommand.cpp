...
std::wstring IndexerCommand::serialize(std::shared_ptr<const IndexerCommand> indexerCommand, bool compact) {
  ...
  return QString::fromUtf8(jsonDocument.toJson(compact ? QJsonDocument::Compact : QJsonDocument::Indented))
      .toStdWString();
}
...
const FilePath& IndexerCommand::getSourceFilePath() const { return m_sourceFilePath; }
