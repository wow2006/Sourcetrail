...
std::vector<SearchResult> SearchIndex::search(const std::wstring& query, NodeTypeSet acceptedNodeTypes,
                                              size_t maxResultCount, size_t maxBestScoredResultsLength) const {
...
void SearchIndex::searchRecursive(const SearchPath& path, const std::wstring& remainingQuery,
                                  NodeTypeSet acceptedNodeTypes, std::vector<SearchIndex::SearchPath>* results) const {
...
std::multiset<SearchResult> SearchIndex::createScoredResults(const std::vector<SearchPath>& paths,
...
SearchResult SearchIndex::bestScoredResult(SearchResult result, std::map<std::wstring, SearchResult>* scoresCache,
  ...
  bestScoredResultRecursive(utility::toLowerCase(result.text), indices, indices.back(), indices.size() - 1, scoresCache,
                            &result);
...
void SearchIndex::bestScoredResultRecursive(const std::wstring& lowerText, const std::vector<size_t>& indices,
                                            const size_t lastIndex, const size_t indicesPos,
...
int SearchIndex::scoreText(const std::wstring& text, const std::vector<size_t>& indices) {
  ...
  for (int i = 0; i < static_cast<int>(indices.size()); i++) {
    ...
    if (i > 0) {
      unmatchedLetterScore += static_cast<int>((indices[i] - indices[i - 1] - 1) * unmatchedLetterBonus);
  ...
  int score = unmatchedLetterScore + consecutiveLetterScore + camelCaseScore + noLetterScore + firstLetterScore +
              leadingStartScore;
...
SearchResult SearchIndex::rescoreText(const std::wstring& fulltext, const std::wstring& text,
                                      const std::vector<size_t>& indices, int score,
                                      size_t maxBestScoredResultsLength) {
