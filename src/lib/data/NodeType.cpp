...
std::vector<NodeType> const NodeType::overviewBundleNodeTypesOrdered = {
    NodeType(NODE_FILE),      NodeType(NODE_MACRO),   NodeType(NODE_ANNOTATION), NodeType(NODE_MODULE),
    NodeType(NODE_NAMESPACE), NodeType(NODE_PACKAGE), NodeType(NODE_CLASS),      NodeType(NODE_INTERFACE),
    NodeType(NODE_STRUCT),    NodeType(NODE_UNION),   NodeType(NODE_FUNCTION),   NodeType(NODE_GLOBAL_VARIABLE),
    NodeType(NODE_TYPE),      NodeType(NODE_TYPEDEF), NodeType(NODE_ENUM)};

NodeType::NodeType(NodeKind kind) : m_kind(kind) {}

bool NodeType::operator==(const NodeType& o) const { return m_kind == o.m_kind; }

bool NodeType::operator!=(const NodeType& o) const { return !operator==(o); }

bool NodeType::operator<(const NodeType& o) const { return m_kind < o.m_kind; }
NodeKind NodeType::getKind() const { return m_kind; }
...
bool NodeType::isInheritable() const {
  ...
  const NodeKindMask mask = NODE_SYMBOL | NODE_BUILTIN_TYPE | NODE_TYPE | NODE_STRUCT | NODE_CLASS | NODE_INTERFACE;
...
bool NodeType::isUsable() const {
  const NodeKindMask mask = NODE_SYMBOL | NODE_BUILTIN_TYPE | NODE_STRUCT | NODE_CLASS | NODE_ENUM | NODE_UNION |
                            NODE_INTERFACE | NODE_ANNOTATION | NODE_TYPEDEF;
  return ((m_kind & mask) > 0);
}

bool NodeType::isPotentialMember() const {
  const NodeKindMask mask = NODE_METHOD | NODE_FIELD | NODE_CLASS | NODE_INTERFACE | NODE_ANNOTATION | NODE_STRUCT |
...
bool NodeType::isCollapsible() const {
  const NodeKindMask mask = NODE_SYMBOL | NODE_TYPE | NODE_BUILTIN_TYPE | NODE_STRUCT | NODE_CLASS | NODE_INTERFACE |
                            NODE_ANNOTATION | NODE_ENUM | NODE_UNION | NODE_FILE;
  return ((m_kind & mask) > 0);
}

bool NodeType::isVisibleAsParentInGraph() const { return !isPackage(); }

bool NodeType::hasSearchFilter() const {
  const NodeKindMask mask = NODE_BUILTIN_TYPE | NODE_MODULE | NODE_NAMESPACE | NODE_PACKAGE | NODE_STRUCT | NODE_CLASS |
                            NODE_INTERFACE | NODE_ANNOTATION | NODE_GLOBAL_VARIABLE | NODE_FIELD | NODE_FUNCTION |
                            NODE_METHOD | NODE_ENUM | NODE_ENUM_CONSTANT | NODE_TYPEDEF | NODE_FILE | NODE_MACRO |
                            NODE_UNION;
  return ((m_kind & mask) > 0);
}
  ...
  switch (m_kind) {
    ...
    case NODE_NAMESPACE: {
      ...
      tree.children.push_back(Tree<BundleInfo>(BundleInfo(
          [](const std::wstring& nodeName) { return nodeName.find(L"anonymous namespace") != std::wstring::npos; },
...
bool NodeType::hasOverviewBundle() const { return !getOverviewBundleTree().data.isValid(); }

std::string NodeType::getUnderscoredTypeString() const {
  return utility::replace(utility::replace(getReadableTypeString(), "-", "_"), " ", "_");
}

std::string NodeType::getReadableTypeString() const { return getReadableNodeKindString(m_kind); }
