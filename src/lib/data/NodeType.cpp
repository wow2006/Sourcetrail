...
bool NodeType::isInheritable() const {
  ...
  const NodeKindMask mask = NODE_SYMBOL | NODE_BUILTIN_TYPE | NODE_TYPE | NODE_STRUCT | NODE_CLASS |
      NODE_INTERFACE;
...
bool NodeType::isUsable() const {
  const NodeKindMask mask = NODE_SYMBOL | NODE_BUILTIN_TYPE | NODE_STRUCT | NODE_CLASS | NODE_ENUM |
      NODE_UNION | NODE_INTERFACE | NODE_ANNOTATION | NODE_TYPEDEF;
  return ((m_kind & mask) > 0);
}
...
bool NodeType::isCollapsible() const {
  const NodeKindMask mask = NODE_SYMBOL | NODE_TYPE | NODE_BUILTIN_TYPE | NODE_STRUCT | NODE_CLASS |
      NODE_INTERFACE | NODE_ANNOTATION | NODE_ENUM | NODE_UNION | NODE_FILE;
  return ((m_kind & mask) > 0);
}
