...
struct StorageFile {
  ...
  StorageFile(Id id, std::wstring filePath, std::wstring languageIdentifier, std::string modificationTime, bool indexed,
              bool complete)
      : id(id),
  ...
  bool operator<(const StorageFile& other) const { return filePath < other.filePath; }
