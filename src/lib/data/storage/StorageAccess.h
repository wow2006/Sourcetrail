...
class StorageAccess {
 public:
  ...
  virtual std::vector<SearchMatch> getAutocompletionMatches(const std::wstring& query, NodeTypeSet acceptedNodeTypes,
  ...
  virtual std::shared_ptr<Graph> getGraphForTrail(Id originId, Id targetId, NodeKindMask nodeTypes,
  ...
  virtual std::shared_ptr<SourceLocationFile> getSourceLocationsOfTypeInFile(const FilePath& filePath,
  ...
  virtual void updateBookmark(const Id bookmarkId, const std::wstring& name, const std::wstring& comment,
