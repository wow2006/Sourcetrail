...
Id PersistentStorage::addError(const StorageErrorData& data) { return m_sqliteIndexStorage.addError(data).m_id; }
...
std::shared_ptr<SourceLocationCollection> PersistentStorage::getSourceLocationsForLocationIds(
  ...
  for (StorageSourceLocation location : m_sqliteIndexStorage.getAllByIds<StorageSourceLocation>(locationIds)) {
    ...
    collection->addSourceLocation(intToLocationType(location.type), location.id, m_locationIdToElementIds[location.id],
                                  getFileNodePath(location.fileNodeId), location.startLine, location.startCol,
                                  location.endLine, location.endCol);
  }
...
std::shared_ptr<SourceLocationFile> PersistentStorage::getSourceLocationsForLinesInFile(const FilePath& filePath,
                                                                                        size_t startLine,
...
Id PersistentStorage::addEdgeBookmark(const EdgeBookmark& bookmark) {
  ...
  for (const Id& edgeId : bookmark.getEdgeIds()) {
    ...
    m_sqliteBookmarkStorage.addBookmarkedEdge(StorageBookmarkedEdgeData(
        ...
        m_sqliteIndexStorage.getNodeById(storageEdge.targetNodeId).serializedName, storageEdge.type, sourceNodeActive));
  }
  return id;
}
...
TooltipSnippet PersistentStorage::getTooltipSnippetForNode(const StorageNode& node) const {
  ...
  if (nameHierarchy.hasSignature()) {
    ...
    while (sigLoc) {
      ...
      if (lines.empty() || lines.size() != sigLoc->getOtherLocation()->getLineNumber() - sigLoc->getLineNumber() + 1 ||
      ...
      file->forEachStartSourceLocation([&sigLoc, &annotations, &lines](SourceLocation* loc) {
        if ((loc->getType() == LOCATION_TOKEN || loc->getType() == LOCATION_QUALIFIER) && sigLoc->contains(*loc)) {
          ...
          for (size_t i = 0; i < loc->getEndLocation()->getLineNumber() - sigLoc->getLineNumber() && i < lines.size();
               i++) {
      ...
      for (const std::pair<Id, std::wstring>& p : annotatedTexts) {
        ...
        if (pos != std::wstring::npos) {
          ...
          snippet.locationFile->addSourceLocation(loc->getType(), loc->getLocationId(), loc->getTokenIds(), 1, pos + 1,
                                                  1, pos + p.second.size());
...
void PersistentStorage::addBundledEdgesToGraph(Id nodeId, const std::vector<StorageEdge>& edgesToBundle,
  ...
  for (const std::pair<Id, std::vector<EdgeInfo>>& p : connectedParentNodeIds) {
    ...
    std::shared_ptr<TokenComponentBundledEdges> componentBundledEdges = std::make_shared<TokenComponentBundledEdges>();
      ...
      std::shared_ptr<std::thread> thread = std::make_shared<std::thread>(
          [&](const std::vector<StorageFile>& files) {
            for (const StorageFile& file : files) {
              m_fullTextSearchIndex.addFile(file.id,
...
void PersistentStorage::buildHierarchyCache() {
  ...
  for (const StorageEdge& edge : memberEdges) {
    ...
    m_hierarchyCache.createConnection(edge.id, edge.sourceNodeId, edge.targetNodeId, sourceIsVisible, sourceIsImplicit,
                                      targetIsImplicit);
  }
