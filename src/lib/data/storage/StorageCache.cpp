...
std::shared_ptr<SourceLocationCollection> StorageCache::getErrorSourceLocations(
  ...
  std::shared_ptr<SourceLocationCollection> collection = StorageAccessProxy::getErrorSourceLocations(
      errors);
...
void StorageCache::addErrorsToCache(const std::vector<ErrorInfo>& newErrors,
