...
public:
  ...
  utility::StorageError addError(const utility::StorageErrorData& data);
...
private:
  ...
  struct TempSourceLocation {
    TempSourceLocation(
        uint32_t startLine_, uint16_t lineDiff_, uint16_t startCol_, uint16_t endCol_, uint8_t type_)
  ...
  utility::LowMemoryStringMap<std::string, uint32_t, 0> m_tempNodeNameIndex;
  utility::LowMemoryStringMap<std::wstring, uint32_t, 0> m_tempWNodeNameIndex;
