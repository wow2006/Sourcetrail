...
void SqliteIndexStorage::setupPrecompiledStatements() {
  try {
    ...
    m_insertElementComponentStmt =
        m_database.compileStatement("INSERT INTO element_component(id, element_id, type, data) VALUES(NULL, ?, ?, ?);");
...
void SqliteIndexStorage::forEach<StorageError>(const std::string& query,
  ...
  while (!q.eof()) {
    ...
    if (id != 0) {
      func(
          StorageError(id, utility::decodeFromUtf8(message), utility::decodeFromUtf8(translationUnit), fatal, indexed));
    }
