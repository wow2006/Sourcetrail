...
int SqliteStorage::executeStatementScalar(const std::string& statement, int nullValue) const {
  int ret = 0;
  try {
...
int SqliteStorage::executeStatementScalar(CppSQLite3Statement& statement, int nullValue) const {
  int ret = 0;
  try {
...
bool SqliteStorage::hasTable(const std::string& tableName) const {
  CppSQLite3Query q = executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='" +
                                   tableName + "';");
