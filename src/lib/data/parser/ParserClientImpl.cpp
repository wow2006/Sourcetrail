...
Id ParserClientImpl::recordSymbol(const NameHierarchy& symbolName) { return addNodeHierarchy(symbolName); }
...
Id ParserClientImpl::recordReference(ReferenceKind referenceKind, Id referencedSymbolId, Id contextSymbolId,
...
void ParserClientImpl::recordComment(const ParseLocation& location) {
  ...
  m_storage->addSourceLocation(
      StorageSourceLocationData(location.fileId, location.startLineNumber, location.startColumnNumber,
                                location.endLineNumber, location.endColumnNumber, locationTypeToInt(LOCATION_COMMENT)));
}

void ParserClientImpl::recordError(const std::wstring& message, bool fatal, bool indexed,
...
bool ParserClientImpl::hasContent() const { return m_storage->getByteSize(1) > 0; }
...
void ParserClientImpl::addSourceLocation(Id elementId, const ParseLocation& location, LocationType type) {
  ...
  Id sourceLocationId = m_storage->addSourceLocation(
      StorageSourceLocationData(location.fileId, location.startLineNumber, location.startColumnNumber,
                                location.endLineNumber, location.endColumnNumber, locationTypeToInt(type)));
