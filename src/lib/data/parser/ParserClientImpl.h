...
class ParserClientImpl : public ParserClient {
 public:
  ...
  Id recordReference(ReferenceKind referenceKind, Id referencedSymbolId, Id contextSymbolId,
  ...
  void recordError(const std::wstring& message, bool fatal, bool indexed, const FilePath& translationUnit,
                   const ParseLocation& location) override;
