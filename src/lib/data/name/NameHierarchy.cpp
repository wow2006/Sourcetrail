...
std::wstring NameHierarchy::serializeRange(const NameHierarchy& nameHierarchy,
                                           size_t first,
                                           size_t last) {
...
NameHierarchy NameHierarchy::deserialize(const std::wstring& serializedName) {
  ...
  while(npos != std::wstring::npos && npos < serializedName.size()) {
    ...
    if(spos == std::wstring::npos) {
      LOG_ERROR(L"unable to deserialize name hierarchy: " +
...
NameHierarchy::NameHierarchy(const std::vector<std::wstring>& names,
                             const NameDelimiterType delimiterType)
