...
Task::TaskState TaskFinishParsing::doUpdate(std::shared_ptr<Blackboard> blackboard) {
  ...
  status +=
      std::to_wstring(indexedSourceFileCount) + L"/" + std::to_wstring(sourceFileCount) + L" source files indexed; ";
  status += utility::decodeFromUtf8(TimeStamp::secondsToString(time));
  status += L"; " + std::to_wstring(errorInfo.m_total) + L" error" + (errorInfo.m_total != 1 ? L"s" : L"");
  ...
  DatabasePolicy policy = m_dialogView->finishedIndexingDialog(
      indexedSourceFileCount, sourceFileCount, stats.completedFileCount, stats.fileCount, static_cast<float>(time),
      errorInfo, interruptedIndexing, shallowIndexing);
...
void TaskFinishParsing::doExit(std::shared_ptr<Blackboard> blackboard) { m_storage.reset(); }
