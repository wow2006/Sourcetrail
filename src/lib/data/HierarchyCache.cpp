...
Id HierarchyCache::HierarchyNode::getNodeId() const { return m_nodeId; }

Id HierarchyCache::HierarchyNode::getEdgeId() const { return m_edgeId; }

void HierarchyCache::HierarchyNode::setEdgeId(Id edgeId) { m_edgeId = edgeId; }

HierarchyCache::HierarchyNode* HierarchyCache::HierarchyNode::getParent() const { return m_parent; }

void HierarchyCache::HierarchyNode::setParent(HierarchyNode* parent) { m_parent = parent; }
...
void HierarchyCache::HierarchyNode::addChild(HierarchyNode* child) { m_children.push_back(child); }

size_t HierarchyCache::HierarchyNode::getChildrenCount() const { return m_children.size(); }
...
bool HierarchyCache::HierarchyNode::isVisible() const { return m_isVisible; }

void HierarchyCache::HierarchyNode::setIsVisible(bool isVisible) { m_isVisible = isVisible; }

bool HierarchyCache::HierarchyNode::isImplicit() const { return m_isImplicit; }

void HierarchyCache::HierarchyNode::setIsImplicit(bool isImplicit) { m_isImplicit = isImplicit; }
...
void HierarchyCache::clear() { m_nodes.clear(); }

void HierarchyCache::createConnection(Id edgeId, Id fromId, Id toId, bool sourceVisible, bool sourceImplicit,
                                      bool targetImplicit) {
  if (fromId == toId) {
    return;
  }
...
HierarchyCache::getInheritanceEdgesForNodeId(Id sourceId, const std::set<Id>& targetIds) const {
  ...
  std::map<Id, std::vector<std::pair<Id, Id>>> reverseGraph = sourceNode->getReverseReachableInheritanceSubgraph();
...
void HierarchyCache::getReverseReachable(
    Id nodeId, const std::map</*target*/ Id, std::vector<std::pair</*source*/ Id, /*edge*/ Id>>>& reverseGraph,
    std::set<Id>& nodes, std::vector<Id>& edges) {
