...
void SourceGroupSettingsWithCxxCdbPath::load(const utility::ConfigManagerigManager* config, const std::string& key)
{
	setCompilationDatabasePath(
		config->getValueOrDefault(key + "/build_file_path/compilation_db_path", FilePath(L"")));
}

void SourceGroupSettingsWithCxxCdbPath::save(utility::ConfigManagerigManager* config, const std::string& key)
{
	config->setValue(
