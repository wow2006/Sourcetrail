...
std::vector<std::wstring> SourceGroupSettingsWithCxxPathsAndFlags::getCompilerFlags() const { return m_compilerFlags; }
...
bool SourceGroupSettingsWithCxxPathsAndFlags::equals(const SourceGroupSettingsBase* other) const {
  ...
  return (otherPtr && utility::isPermutation(m_headerSearchPaths, otherPtr->m_headerSearchPaths) &&
...
void SourceGroupSettingsWithCxxPathsAndFlags::load(const ConfigManager* config, const std::string& key) {
  setHeaderSearchPaths(
      config->getValuesOrDefaults(key + "/header_search_paths/header_search_path", std::vector<FilePath>()));
  setFrameworkSearchPaths(
      config->getValuesOrDefaults(key + "/framework_search_paths/framework_search_path", std::vector<FilePath>()));
