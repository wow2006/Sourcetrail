...
void SourceGroupSettingsWithSourceExtensions::load(const utility::ConfigManager* config,
                                                   const std::string& key) {
  setSourceExtensions(config->getValuesOrDefaults(
      key + "/source_extensions/source_extension", std::vector<std::wstring>()));
}

void SourceGroupSettingsWithSourceExtensions::save(utility::ConfigManager* config,
                                                   const std::string& key) {
