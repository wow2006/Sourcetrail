...
SourceGroupSettingsUnloadable::SourceGroupSettingsUnloadable(const std::string& id,
                                                             const ProjectSettings* projectSettings)
    : SourceGroupSettings(SOURCE_GROUP_UNKNOWN, id, projectSettings) {}

std::string SourceGroupSettingsUnloadable::getTypeString() { return m_typeString; }
...
bool SourceGroupSettingsUnloadable::equalsSettings(const SourceGroupSettingsBase* other) {
  ...
  if (const SourceGroupSettingsUnloadable* otherUnloadable =
    ...
    for (auto it : m_content) {
      ...
      if (otherIt == otherUnloadable->m_content.end() || !utility::isPermutation(it.second, otherIt->second)) {
        return false;
      }
    }
    return true;
