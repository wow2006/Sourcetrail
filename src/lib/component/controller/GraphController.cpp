...
void GraphController::handleMessage(MessageActivateOverview* message) {
  ...
  if (message->acceptedNodeTypes != NodeTypeSet::all()) {
    createDummyGraphAndSetActiveAndVisibility(std::vector<Id>(),
                                              m_storageAccess->getGraphForNodeTypes(message->acceptedNodeTypes), false);
        ...
        for (size_t i = 0, l = m_dummyEdges.size(); i < l; i++) {
          ...
          if (edge && edge->data && edge->data->isType(Edge::EDGE_BUNDLED_EDGES) &&
            ...
            std::vector<Id> bundledEdgesIds =
                utility::toVector<Id>(edge->data->getComponent<TokenComponentBundledEdges>()->getBundledEdgesIds());
    ...
    node->data->forEachNodeRecursive([&bundleInfo](const Node* n) {
      ...
      n->forEachEdgeOfType(~Edge::EDGE_MEMBER, [&bundleInfo, &n](Edge* e) {
        ...
        if (e->isType(Edge::EDGE_BUNDLED_EDGES)) {
          TokenComponentBundledEdges::Direction dir = e->getComponent<TokenComponentBundledEdges>()->getDirection();
  ...
  std::shared_ptr<DummyNode> bundleNode = bundleNodesMatching(
      ...
      [&](const DummyNode* node) {
        return (considerInvisibleNodes || node->visible) && node->isGraphNode() && node->data->getType() == type &&
               bundleInfoTree.data.m_nameMatcher(node->name);
      },
...
void GraphController::createLegendGraph() {
  ...
  auto addNode = [&id, &graph, &nodePositions](NodeKind kind, const std::wstring& name, Vec2i position,
  ...
  // Edges
  {
...

    {
      ...
      Node* templateFunctionNode =
          addNode(NODE_FUNCTION, L"template_function<typename ParameterType>", Vec2i(x, y + dy * ++i));
      y += 20;
      Node* templateFunctionSpecializationNode =
          addNode(NODE_FUNCTION, L"template_function<ArgumentType>", Vec2i(x, y + dy * ++i), DEFINITION_IMPLICIT);
      ...
      Node* templateSpecializationNode =
          addNode(NODE_TYPE, L"TemplateType<ArgumentType>", Vec2i(x, y + dy * ++i), DEFINITION_IMPLICIT);
...

    {
      ...
      Node* templateSpecializationNode =
          addNode(NODE_TYPE, L"TemplateType<ArgumentType>", Vec2i(x, y + dy * ++i + 20), DEFINITION_IMPLICIT);
