...
void ErrorController::handleMessage(MessageIndexingStarted* message) { m_storageAccess->setUseErrorCache(true); }

void ErrorController::handleMessage(MessageShowError* message) { getView()->setErrorId(message->errorId); }

ErrorView* ErrorController::getView() const { return Controller::getView<ErrorView>(); }
