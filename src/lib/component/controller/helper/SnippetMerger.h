...
class SnippetMerger {
    ...
    static ContainerType<Range, std::allocator<Range>> mergeAdjacent(ContainerType<Range, std::allocator<Range>> ranges,
      ...
      for (size_t i = 0; i + 1 < ranges.size(); i++) {
        ...
        if (first.end.row + rowDifference >= second.start.row) {
          ...
          ranges.insert(ranges.begin() + i, Range(first.start.row < second.start.row ? first.start : second.start,
 ...
 private:
  Range getExpandedRegardingAtomicRanges(Range range, const int snippetExpandRange,
