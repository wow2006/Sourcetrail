...
void SnippetMerger::addChild(std::shared_ptr<SnippetMerger> child) { m_children.push_back(child); }
...
std::deque<SnippetMerger::Range> SnippetMerger::merge(std::vector<SnippetMerger::Range> atomicRanges) const {
  ...
  if (m_children.size() == 0) {
    ...
    std::sort(merged.begin(), merged.end(), [](const Range& a, const Range& b) { return a.start.row < b.start.row; });
...
SnippetMerger::Range SnippetMerger::getExpandedRegardingAtomicRanges(Range range, const int snippetExpandRange,
