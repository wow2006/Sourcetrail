...
void BookmarkController::createBookmark(const std::wstring& name, const std::wstring& comment,
                                        const std::wstring& category, Id nodeId) {
  ...
  if (!nodeId ||
      (m_activeNodeIds[TabId::currentTab()].size() == 1 && m_activeNodeIds[TabId::currentTab()][0] == nodeId)) {
...
void BookmarkController::editBookmark(Id bookmarkId, const std::wstring& name, const std::wstring& comment,
...
void BookmarkController::handleActivation(const MessageActivateBase* message) { clear(); }
...
void BookmarkController::handleMessage(MessageBookmarkActivate* message) { activateBookmark(message->bookmark); }

void BookmarkController::handleMessage(MessageBookmarkBrowse* message) {
  displayBookmarksFor(message->filter, message->order);
}

void BookmarkController::handleMessage(MessageBookmarkCreate* message) { showBookmarkCreator(message->nodeId); }

void BookmarkController::handleMessage(MessageBookmarkDelete* message) { deleteBookmarkForActiveTokens(); }

void BookmarkController::handleMessage(MessageBookmarkEdit* message) { showBookmarkCreator(0); }
...
void BookmarkController::update() {
  ...
  std::vector<std::shared_ptr<Bookmark>> bookmarks =
      getBookmarks(Bookmark::FILTER_ALL, Bookmark::ORDER_DATE_DESCENDING);
