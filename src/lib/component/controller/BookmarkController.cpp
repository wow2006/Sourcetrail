...
void BookmarkController::displayBookmarksFor(Bookmark::BookmarkFilter filter,
                                             Bookmark::BookmarkOrder order) {
...
void BookmarkController::activateBookmark(const std::shared_ptr<Bookmark> bookmark) {
  ...
  if(std::shared_ptr<EdgeBookmark> edgeBookmark = std::dynamic_pointer_cast<EdgeBookmark>(bookmark)) {
    if(!edgeBookmark->getEdgeIds().empty()) {
      ...
      if(edgeBookmark->getEdgeIds().size() == 1) {
        ...
        MessageActivateEdge(firstEdgeId, Edge::intToType(storageEdge.type), sourceName, targetName)
            .dispatch();
      } else {
  ...
  } else if(std::shared_ptr<NodeBookmark> nodeBookmark = std::dynamic_pointer_cast<NodeBookmark>(
                bookmark)) {
...
bool BookmarkController::bookmarkDateCompare(const std::shared_ptr<Bookmark> a,
                                             const std::shared_ptr<Bookmark> b) {
  return a->getTimeStamp() < b->getTimeStamp();
}

bool BookmarkController::bookmarkNameCompare(const std::shared_ptr<Bookmark> a,
                                             const std::shared_ptr<Bookmark> b) {
