...
void TooltipController::handleMessage(MessageActivateTokens* message) { clear(); }

void TooltipController::handleMessage(MessageActivateLocalSymbols* message) { clear(); }
...
void TooltipController::handleMessage(MessageFocusOut* message) { requestTooltipHide(); }

void TooltipController::handleMessage(MessageGraphNodeExpand* message) { clear(); }

void TooltipController::handleMessage(MessageScrollCode* message) { clear(); }

void TooltipController::handleMessage(MessageScrollGraph* message) { clear(); }

void TooltipController::handleMessage(MessageTooltipHide* message) { clear(); }
...
void TooltipController::handleMessage(MessageTooltipShow* message) {
  if (!message->tooltipInfo.title.empty()) {
    ...
    TooltipInfo info = m_storageAccess->getTooltipInfoForSourceLocationIdsAndLocalSymbolIds(message->sourceLocationIds,
                                                                                            message->localSymbolIds);
...
void TooltipController::handleMessage(MessageWindowFocus* message) { clear(); }

TooltipView* TooltipController::getView() const { return Controller::getView<TooltipView>(); }
...
void TooltipController::requestTooltipShow(const std::vector<Id> tokenIds, TooltipInfo info, TooltipOrigin origin) {
  ...
  Task::dispatch(
      TabId::app(),
      std::make_shared<TaskDecoratorDelay>(delayMS)->addChildTask(std::make_shared<TaskLambda>([requestId, this]() {
