...
void CustomTrailController::clear() {
  getView()->clearView();
  getView()->setAvailableNodeAndEdgeTypes(m_storageAccess->getAvailableNodeTypes(),
                                          m_storageAccess->getAvailableEdgeTypes());
}
...
void CustomTrailController::handleMessage(MessageCustomTrailShow* message) { getView()->showView(); }

void CustomTrailController::handleMessage(MessageIndexingFinished* message) { clear(); }

void CustomTrailController::handleMessage(MessageWindowClosed* message) { getView()->hideView(); }

CustomTrailView* CustomTrailController::getView() { return Controller::getView<CustomTrailView>(); }
