...
Id UndoRedoController::getSchedulerId() const { return Controller::getTabId(); }

UndoRedoView* UndoRedoController::getView() { return Controller::getView<UndoRedoView>(); }
...
void UndoRedoController::handleMessage(MessageActivateErrors* message) {
  if (sameMessageTypeAsLast(message) && static_cast<MessageActivateErrors*>(lastMessage())->filter == message->filter &&
...
void UndoRedoController::handleMessage(MessageActivateTokens* message) {
  ...
  Command command(std::make_shared<MessageActivateTokens>(*message),
...
void UndoRedoController::handleMessage(MessageActivateTrail* message) {
  ...
  Command command(std::make_shared<MessageActivateTrail>(*message),
...
void UndoRedoController::handleMessage(MessageFocusChanged* message) {
  if (sameMessageTypeAsLast(message) &&
      static_cast<MessageFocusChanged*>(lastMessage())->tokenOrLocationId == message->tokenOrLocationId) {
    return;
  }
...
void UndoRedoController::handleMessage(MessageShowError* message) {
  if (sameMessageTypeAsLast(message) && static_cast<MessageShowError*>(lastMessage())->errorId == message->errorId) {
    return;
  }
...
MessageBase* UndoRedoController::lastMessage() const { return std::prev(m_iterator)->message.get(); }
