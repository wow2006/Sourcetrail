...
void parseAndSetValue(boolFunc func, const char* opt, ApplicationSettings* settings, po::variables_map& vm) {
  if(vm.count(opt)) {
    (settings->*func)(vm[opt].as<bool>());
  }
}
...
void CommandlineCommandConfig::setup() {
  ...
  options.add_options()("help,h", "Print this help message")(
      ...
      "separated)")("global-header-search-paths,g",
                    po::value<std::vector<std::string>>(),
                    "Global include paths (once per path or comma separated)")(
      "global-framework-search-paths,F",
      po::value<std::vector<std::string>>(),
