...
std::function<Ret(Args...)> loadFunctionFromLibrary(const FilePath& libraryPath, const std::string& functionName,
  ...
  if (!functionId) {
    errorString =
        "Could not locate the function \"" + functionName + "\" in library\"" + libraryPathString.toStdString() + "\"";
    return std::function<Ret(Args...)>();
  }
#else
