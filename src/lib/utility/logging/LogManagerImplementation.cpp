...
void LogManagerImplementation::operator=(const LogManagerImplementation& other) { m_loggers = other.m_loggers; }
...
void LogManagerImplementation::clearLoggers() { m_loggers.clear(); }
...
void LogManagerImplementation::logInfo(const std::wstring& message, const std::string& file,
...
void LogManagerImplementation::logWarning(const std::wstring& message, const std::string& file,
                                          const std::string& function, const unsigned int line) {
  std::lock_guard<std::mutex> lockGuardLogger(m_loggerMutex);
  for (unsigned int i = 0; i < m_loggers.size(); i++) {
...
void LogManagerImplementation::logError(const std::wstring& message, const std::string& file,
                                        const std::string& function, const unsigned int line) {
  std::lock_guard<std::mutex> lockGuardLogger(m_loggerMutex);
  for (unsigned int i = 0; i < m_loggers.size(); i++) {
