    ...
    if(message->getType() == MessageErrorCountUpdate::getStaticType()) {
      for(auto it = messageBuffer->begin() + 1; it != messageBuffer->end(); it++) {
        if((*it)->getType() == MessageErrorCountUpdate::getStaticType()) {
          MessageErrorCountUpdate* frontErrorsMessage = dynamic_cast<MessageErrorCountUpdate*>(
              message);
          MessageErrorCountUpdate* backErrorsMessage = dynamic_cast<MessageErrorCountUpdate*>(
              it->get());

          backErrorsMessage->newErrors.insert(backErrorsMessage->newErrors.begin(),
