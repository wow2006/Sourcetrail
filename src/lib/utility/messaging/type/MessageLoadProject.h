...
class MessageLoadProject : public Message<MessageLoadProject> {
 public:
  MessageLoadProject(const FilePath& filePath, bool settingsChanged = false,
                     RefreshMode refreshMode = RefreshMode::REFRESH_NONE, bool shallowIndexingRequested = false)
      : projectSettingsFilePath(filePath),
        settingsChanged(settingsChanged),
        refreshMode(refreshMode),
        shallowIndexingRequested(shallowIndexingRequested) {}

  static const std::string getStaticType() { return "MessageLoadProject"; }
