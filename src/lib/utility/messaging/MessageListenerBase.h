...
class MessageListenerBase {
 public:
  MessageListenerBase() : m_id(s_nextId++), m_alive(true) { MessageQueue::getInstance()->registerListener(this); }
  ...
  void removedListener() { m_alive = false; }

  virtual Id getSchedulerId() const { return 0; }
