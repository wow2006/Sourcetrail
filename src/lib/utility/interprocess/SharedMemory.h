...
class SharedMemory {
 public:
  enum AccessMode { CREATE_AND_DELETE, OPEN_ONLY, OPEN_OR_CREATE };

  using Allocator = boost::interprocess::managed_shared_memory::segment_manager;

  using String =
      boost::interprocess::basic_string<char, std::char_traits<char>, boost::interprocess::allocator<char, Allocator>>;
  ...
  using Map =
      boost::interprocess::map<T, T2, std::less<T>, boost::interprocess::allocator<std::pair<const T, T2>, Allocator>>;
