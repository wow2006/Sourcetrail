...
SharedMemory::ScopedAccess::ScopedAccess(SharedMemory* memory)
  ...
  try {
    ...
    LOG_ERROR_STREAM(<< "boost exception thrown at ScopedAccess constructor - "
                     << memory->getMemoryName() << ": " << e.what());
...
SharedMemory::SharedMemory(const std::string& name, size_t initialMemorySize, AccessMode mode)
  ...
  try {
    ...
    case CREATE_AND_DELETE: {
      auto* collector = SharedMemoryGarbageCollector::getInstance();
    ...
    LOG_ERROR_STREAM(<< "boost exception thrown at shared memory creation - " << getMemoryName()
                     << ": " << e.what());
    throw e;
  }
}
