...
private:
  inline void checkIndexInRange(uint32_t index, const std::string& function) const {
    if(index >= N) {
      throw std::range_error(
          fmt::format("{}: index {} is out of range, maximum is {}", function, index, N - 1));
    }
  }
...
template <class T, uint32_t N>
T VectorBase<T, N>::getValue(const uint32_t index) const {
...
template <class T, uint32_t N>
void VectorBase<T, N>::setValue(const uint32_t index, const T& value) {
  VECTOR_CHECK_INDEX(index);
...
template <class T, uint32_t N>
uint32_t VectorBase<T, N>::getDimensions() const {
  return N;
}
...
template <class T, uint32_t N>
T VectorBase<T, N>::operator[](const uint32_t index) {
  VECTOR_CHECK_INDEX(index);
