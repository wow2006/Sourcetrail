...
class StringTraits<std::string> {
 public:
  ...
  static size_t SizeFn(const CharT* str) { return strlen(str); }
...
class StringTraits<std::wstring> {
 public:
  ...
  static size_t SizeFn(const CharT* str) { return wcslen(str); }
...
class LowMemoryStringMap {
 public:
   ...
   */
  ValueT find(const StringT& str) const { return m_root.find(str, 0); }

  void print(StreamT& os) const { m_root.print(os, 0); }

  /*
   * Returns the number of bytes used to store this map.
   */
  size_t getByteSize() const { return m_root.getByteSize(); }

  /*
   * Returns the number of bytes necessary to store all raw string - value pairs that were added.
   */
  size_t getUncompressedByteSize() const { return m_uncompressedByteSize; }
 ...
 private:
  ...
  class LongString {
   public:
    ...
    StringT getString() const { return StringT(m_str.get()); }
  ...
  class ShortString {
   public:
    ShortString(const StringT& str) { StringTraits<StringT>::CopyFn(m_str, str.c_str(), Size); }

    StringT getString() const { return StringT(m_str, Size); }
  ...
  class EmptyString {
   public:
    ...
    StringT getString() const { return StringT(); }

    std::pair<bool, size_t> compareString(const StringT& str, size_t idx) const { return std::make_pair(true, 0); }
  ...
  class Node {
   public:
    ...
    virtual ValueT getValue() const { return defaultVal; }
  ...
  class Leaf : public Node {
   public:
    ...
    ValueT getValue() const override { return m_value; }
   ...
   */
  template <typename StringType>
  class StringBranch : public Branch, public StringType {
   public:
    ...
    size_t getByteSize() const override { return sizeof(*this) + Branch::getByteSize() + StringType::getByteSize(); }
