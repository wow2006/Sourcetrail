...
Project::Project(std::shared_ptr<ProjectSettings> settings, StorageCache* storageCache, std::string appUUID,
                 bool hasGUI)
...
void Project::refresh(std::shared_ptr<DialogView> dialogView, RefreshMode refreshMode, bool shallowIndexingRequested) {
  ...
  if (m_hasGUI) {
    ...
    if (!needsFullRefresh) {
      enabledModes.insert(enabledModes.end(),
                          {RefreshMode::REFRESH_UPDATED_FILES, RefreshMode::REFRESH_UPDATED_AND_INCOMPLETE_FILES});
    }
...
void Project::buildIndex(RefreshInfo info, std::shared_ptr<DialogView> dialogView) {
  ...
  if (info.mode != RefreshMode::REFRESH_ALL_FILES && (info.filesToClear.size() || info.nonIndexedFilesToClear.size())) {
    for (const std::shared_ptr<SourceGroup>& sourceGroup : m_sourceGroups) {
...
bool Project::hasCxxSourceGroup() const {
  ...
  return std::any_of(m_sourceGroups.cbegin(), m_sourceGroups.cend(), [](const auto& sourceGroup) {
    return sourceGroup->getLanguage() == LANGUAGE_C || sourceGroup->getLanguage() == LANGUAGE_CPP;
  });
