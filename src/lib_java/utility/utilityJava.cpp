...
std::vector<std::wstring> getRequiredJarNames() {
  return {L"gradle-tooling-api-6.7.1.jar",
          L"java-indexer.jar",
...
std::set<FilePath> fetchRootDirectories(const std::set<FilePath>& sourceFilePaths) {
  ...
  for (const FilePath& filePath : sourceFilePaths) {
    ...
    javaEnvironment->callStaticStringMethod("com/sourcetrail/JavaIndexer", "getPackageName", packageName,
                                            textAccess->getText());
    ...
    for (std::vector<std::string>::const_reverse_iterator it = packageNameParts.rbegin(); it != packageNameParts.rend();
         it++) {
...
#pragma warning(pop)
  {
    ...
    LOG_WARNING("Environment variable \"JAVA_HOME\" not found on system. Setting value to \"" + javaHomePath.str() +
                "\" for this process.");
#pragma warning(push)
#pragma warning(disable : 4996)
