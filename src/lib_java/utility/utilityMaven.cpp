...
namespace {
void fetchDirectories(std::vector<FilePath>& pathList, std::shared_ptr<TextAccess> xmlAccess,
                      const std::vector<std::string>& tags, const FilePath& toAppend = FilePath()) {
  {
  ...
  std::vector<std::string> fetchedDirectories = utility::getValuesOfAllXmlElementsOnPath(xmlAccess, tags);
...
namespace utility {
std::wstring mavenGenerateSources(const FilePath& mavenPath, const FilePath& settingsFilePath,
  ...
  std::shared_ptr<TextAccess> outputAccess = TextAccess::createFromString(
      utility::encodeToUtf8(utility::executeProcess(mavenPath.wstr(), args, projectDirectoryPath, true, 60000).output));
...
bool mavenCopyDependencies(const FilePath& mavenPath, const FilePath& settingsFilePath,
                           const FilePath& projectDirectoryPath, const FilePath& outputDirectoryPath) {
  utility::setJavaHomeVariableIfNotExists();
...
std::vector<FilePath> mavenGetAllDirectoriesFromEffectivePom(const FilePath& mavenPath,
  ...
  if (addTestDirectories) {
    ...
    fetchDirectories(
        uncheckedDirectories, xmlAccess,
  ...
  LOG_INFO("Found " + std::to_string(directories.size()) + " of " + std::to_string(uncheckedDirectories.size()) +
