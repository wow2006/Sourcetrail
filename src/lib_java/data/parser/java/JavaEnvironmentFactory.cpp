...
void JavaEnvironmentFactory::createInstance(std::string classPath, std::string& errorString) {
  ...
  createInstanceFunction =
      utility::loadFunctionFromLibrary<jint, JavaVM**, void**, void*>(javaPath, "JNI_CreateJavaVM", errorString);
...
std::shared_ptr<JavaEnvironmentFactory> JavaEnvironmentFactory::getInstance() { return s_instance; }
...
void JavaEnvironmentFactory::unregisterEnvironment() {
  std::thread::id currentThreadId = std::this_thread::get_id();
  {
    ...
    if (it != m_threadIdToEnvAndUserCount.end()) {
      it->second.second--;
      if (it->second.second == 0) {  // TODO: currently this happens quite often. do something about that.
