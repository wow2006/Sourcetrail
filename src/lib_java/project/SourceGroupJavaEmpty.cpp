...
std::vector<FilePath> SourceGroupJavaEmpty::doGetClassPath() const {
  return utility::getClassPath(m_settings->getClasspathExpandedAndAbsolute(), m_settings->getUseJreSystemLibrary(),
                               getAllSourceFilePaths());
}

std::shared_ptr<SourceGroupSettings> SourceGroupJavaEmpty::getSourceGroupSettings() { return m_settings; }

std::shared_ptr<const SourceGroupSettings> SourceGroupJavaEmpty::getSourceGroupSettings() const { return m_settings; }
