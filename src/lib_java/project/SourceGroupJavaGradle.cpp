...
SourceGroupJavaGradle::SourceGroupJavaGradle(std::shared_ptr<SourceGroupSettingsJavaGradle> settings)
    : m_settings(settings), m_allSourcePathsCache(std::bind(&SourceGroupJavaGradle::doGetAllSourcePaths, this)) {}
...
std::vector<FilePath> SourceGroupJavaGradle::getAllSourcePaths() const { return m_allSourcePathsCache.getValue(); }
...
std::vector<FilePath> SourceGroupJavaGradle::doGetClassPath() const {
  ...
  if (m_settings->getGradleDependenciesDirectoryPath().exists()) {
    std::vector<FilePath> gradleJarPaths =
        FileSystem::getFilePathsFromDirectory(m_settings->getGradleDependenciesDirectoryPath(), {L".jar"});
...
std::shared_ptr<SourceGroupSettings> SourceGroupJavaGradle::getSourceGroupSettings() { return m_settings; }

std::shared_ptr<const SourceGroupSettings> SourceGroupJavaGradle::getSourceGroupSettings() const { return m_settings; }
...
bool SourceGroupJavaGradle::prepareGradleData() {
  if (m_settings && m_settings->getGradleProjectFilePathExpandedAndAbsolute().exists()) {
    ...
    bool success = utility::gradleCopyDependencies(projectRootPath, m_settings->getGradleDependenciesDirectoryPath(),
