...
SourceGroupJavaMaven::SourceGroupJavaMaven(std::shared_ptr<SourceGroupSettingsJavaMaven> settings)
    : m_settings(settings), m_allSourcePathsCache(std::bind(&SourceGroupJavaMaven::doGetAllSourcePaths, this)) {}
...
std::vector<FilePath> SourceGroupJavaMaven::getAllSourcePaths() const { return m_allSourcePathsCache.getValue(); }
...
std::vector<FilePath> SourceGroupJavaMaven::doGetClassPath() const {
  ...
  if (m_settings && m_settings->getMavenDependenciesDirectoryPath().exists()) {
    std::vector<FilePath> mavenJarPaths =
        FileSystem::getFilePathsFromDirectory(m_settings->getMavenDependenciesDirectoryPath(), {L".jar"});
...
std::shared_ptr<SourceGroupSettings> SourceGroupJavaMaven::getSourceGroupSettings() { return m_settings; }

std::shared_ptr<const SourceGroupSettings> SourceGroupJavaMaven::getSourceGroupSettings() const { return m_settings; }
...
bool SourceGroupJavaMaven::prepareMavenData() {
  if (m_settings && m_settings->getMavenProjectFilePathExpandedAndAbsolute().exists()) {
    ...
    std::shared_ptr<DialogView> dialogView =
        Application::getInstance()->getDialogView(DialogView::UseCase::PROJECT_SETUP);
    ...
    bool success = utility::mavenCopyDependencies(mavenPath, mavenSettingsPath, projectRootPath,
...
std::vector<FilePath> SourceGroupJavaMaven::doGetAllSourcePaths() const {
  ...
  if (m_settings && m_settings->getMavenProjectFilePathExpandedAndAbsolute().exists()) {
    ...
    sourcePaths = utility::mavenGetAllDirectoriesFromEffectivePom(mavenPath, mavenSettingsPath, projectRootPath,
                                                                  m_settings->getMavenDependenciesDirectoryPath(),
