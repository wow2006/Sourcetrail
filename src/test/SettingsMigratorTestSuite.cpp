...
TEST_CASE("migrator with lambda") {
  ...
  migrator.addMigration(
      1, std::make_shared<SettingsMigrationLambda>([](const SettingsMigration* migration, Settings* settings) {
        migration->setValueInSettings<int>(settings, "value",
                                           migration->getValueFromSettings<int>(settings, "value", 0) * 2);
      }));
  migrator.migrate(&settingsBefore, 1);
...
TEST_CASE("migrator with multiple lambdas") {
  ...
  migrator.addMigration(
      2, std::make_shared<SettingsMigrationLambda>([](const SettingsMigration* migration, Settings* settings) {
        migration->setValueInSettings<int>(settings, "value",
                                           migration->getValueFromSettings<int>(settings, "value", 0) - 1);
      }));
  migrator.migrate(&settingsBefore, 2);
