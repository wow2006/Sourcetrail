...
class SourceGroupSettingsTest : public SourceGroupSettings {
 public:
  ...
  std::shared_ptr<SourceGroupSettings> createCopy() const override { return nullptr; }

  void loadSettings(const ConfigManager* config) override {}

  void saveSettings(ConfigManager* config) override {}

  bool equalsSettings(const SourceGroupSettingsBase* other) override { return true; }
};
...
class SourceGroupTest : public SourceGroup {
 public:
  ...
  std::set<FilePath> getAllSourceFilePaths() const override { return m_sourceFilePaths; }
  ...
  void setStatus(SourceGroupStatusType status) { m_sourceGroupSettings->setStatus(status); }

 private:
  std::shared_ptr<SourceGroupSettings> getSourceGroupSettings() override { return m_sourceGroupSettings; }

  std::shared_ptr<const SourceGroupSettings> getSourceGroupSettings() const override { return m_sourceGroupSettings; }
...
Id addFileToStorage(const FilePath& filePath, const std::string& modificationTime, bool indexed, bool complete,
                    std::shared_ptr<PersistentStorage> storage) {
  const Id id = storage
                    ->addNode(StorageNodeData(
                        NODE_FILE, NameHierarchy::serialize(NameHierarchy(filePath.wstr(), NAME_DELIMITER_FILE))))
                    .first;
  storage->addFile(StorageFile(id, filePath.wstr(), L"someLanguage", modificationTime, indexed, complete));
  return id;
}

Id addVeryOldFileToStorage(const FilePath& filePath, bool indexed, bool complete,
                           std::shared_ptr<PersistentStorage> storage) {
  return addFileToStorage(filePath,
                          QDateTime::currentDateTime().addDays(-1).toString("yyyy-MM-dd hh:mm:ss").toStdString(),
                          indexed, complete, storage);
}

Id addVeryNewFileToStorage(const FilePath& filePath, bool indexed, bool complete,
                           std::shared_ptr<PersistentStorage> storage) {
  return addFileToStorage(filePath,
                          QDateTime::currentDateTime().addDays(1).toString("yyyy-MM-dd hh:mm:ss").toStdString(),
                          indexed, complete, storage);
...
TEST_CASE("refresh info for all files is empty for empty project") {
  ...
  REQUIRE(RefreshMode::REFRESH_ALL_FILES == refreshInfo.mode);
...
TEST_CASE("refresh info for all files clears nothing and indexes previously unknown source file") {
  cleanup();
  {
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), sourceFilePath));
  }
  cleanup();
}
...
TEST_CASE("refresh info for all files is clears indexed files of disabled source group") {
  cleanup();
  {
    ...
    REQUIRE(RefreshMode::REFRESH_UPDATED_FILES == refreshInfo.mode);
    REQUIRE(0 == refreshInfo.nonIndexedFilesToClear.size());
    REQUIRE(1 == refreshInfo.filesToClear.size());
    REQUIRE(0 == refreshInfo.filesToIndex.size());

    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), sourceFilePath));
  }
  cleanup();
}
...
TEST_CASE("refresh info for all files is clears nonindexed files of disabled source group") {
  cleanup();
  {
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), upToDateSourceFilePath));

    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.nonIndexedFilesToClear),
...
namespace {
enum KnownState { UNKNOWN, NON_INDEXED, INDEXED };
enum ChangedState { UNCHANGED, CHANGED };
enum FileState { SOURCE_FILE, HEADER_FILE };
enum IndexingState { NOT_TO_INDEX, TO_INDEX };

RefreshInfo getRefreshInfo(KnownState knownState, ChangedState changedState, FileState fileState,
...
TEST_CASE(
  ...
  cleanup();
  {
    ...
    sourceGroups.push_back(
        std::shared_ptr<SourceGroupTest>(new SourceGroupTest({upToDateSourceFilePath, outdatedSourceFilePath})));
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), upToDateSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), outdatedSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), upToDateSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), outdatedSourceFilePath));
  }
  cleanup();
}
...
TEST_CASE(
  ...
  cleanup();
  {
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), upToDateSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), outdatedHeaderFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), upToDateSourceFilePath));
  }
  cleanup();
}
...
TEST_CASE("refresh info for updated files clears outdated source file and referenced uptodate header file") {
  cleanup();
  {
    const FilePath outdatedSourceFilePath = m_sourceFolder.getConcatenated(L"outdated_file.cpp");
    const FilePath upToDateHeaderFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.h");
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), outdatedSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), upToDateHeaderFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), outdatedSourceFilePath));
  }
  cleanup();
}

TEST_CASE("refresh info for updated files does not clear uptodate header referenced by uptodate source") {
  cleanup();
  {
    const FilePath outdatedSourceFilePath = m_sourceFolder.getConcatenated(L"outdated_file.cpp");
    const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
...
TEST_CASE("clears unchanged files referenced by unchanged file that referenced changed indexed file") {
  cleanup();
  {
    const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
    const FilePath upToDateHeaderFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.h");
    ...
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.nonIndexedFilesToClear),
                                               upToDateHeaderFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), upToDateSourceFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToClear), outOfDateHeaderFilePath));
    REQUIRE(utility::containsElement<FilePath>(utility::toVector(refreshInfo.filesToIndex), upToDateSourceFilePath));
  }
  cleanup();
}

TEST_CASE("clears unchanged files referenced by unchanged file that referenced changed nonindexed file") {
  cleanup();
  {
    const FilePath upToDateSourceFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.cpp");
    const FilePath upToDateHeaderFilePath = m_sourceFolder.getConcatenated(L"up_to_date_file.h");
