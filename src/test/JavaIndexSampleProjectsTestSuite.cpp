...
std::shared_ptr<TextAccess> parseCode(const FilePath& sourceFilePath, const FilePath& projectDataSrcRoot,
  ...
  std::shared_ptr<IndexerCommandJava> command = std::make_shared<IndexerCommandJava>(sourceFilePath, L"12", classpath);
...
void processSourceFile(const std::string& projectName, const FilePath& sourceFilePath,
  ...
  std::shared_ptr<TextAccess> output =
      parseCode(projectDataSrcRoot.getConcatenated(sourceFilePath), projectDataSrcRoot, classpath);
  ...
  if (updateExpectedOutput || !expectedOutputFilePath.exists()) {
    ...
    REQUIRE_MESSAGE(("Output does not match the expected line count for file " + sourceFilePath.str() + " in project " +
...
TEST_CASE("index javasymbolsolver 0 6 0 project") {
  ...
  processSourceFiles("JavaSymbolSolver060",
