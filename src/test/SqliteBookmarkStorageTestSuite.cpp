...
TEST_CASE("add bookmarks") {
  ...
  int result = -1;
  {
    ...
    for (size_t i = 0; i < bookmarkCount; i++) {
      ...
      storage.addBookmark(
          StorageBookmarkData(L"test bookmark", L"test comment", TimeStamp::now().toString(), categoryId));
    }
...
TEST_CASE("add bookmarked node") {
  ...
  int result = -1;
  {
    ...
    const Id categoryId = storage.addBookmarkCategory(StorageBookmarkCategoryData(L"test category")).id;
    const Id bookmarkId = storage
                              .addBookmark(StorageBookmarkData(L"test bookmark", L"test comment",
                                                               TimeStamp::now().toString(), categoryId))
                              .id;
