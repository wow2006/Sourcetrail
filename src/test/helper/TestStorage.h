...
class TestStorage {
  ...
  static std::shared_ptr<TestStorage> create(std::shared_ptr<const Storage> storage) {
    ...
    for (const StorageNode& node : storage->getStorageNodes()) {
      ...
      for (auto qualifierLocationIt = qualifierLocationMap.find(node.id);
        ...
        testStorage->addLine(L"QUALIFIER: " + nameStr +
      ...
      if (bin != nullptr) {
        ...
        for (auto tokenLocationIt = tokenLocationMap.find(node.id);
             tokenLocationIt != tokenLocationMap.end() && tokenLocationIt->first == node.id; tokenLocationIt++) {
          added = false;
          ...
          for (auto scopeLocationIt = scopeLocationMap.find(node.id);
               scopeLocationIt != scopeLocationMap.end() && scopeLocationIt->first == node.id; scopeLocationIt++) {
    ...
    for (const StorageEdge& edge : storage->getStorageEdges()) {
      ...
      if (bin != nullptr) {
        ...
        for (auto tokenLocationIt = tokenLocationMap.find(edge.id);
             tokenLocationIt != tokenLocationMap.end() && tokenLocationIt->first == edge.id; tokenLocationIt++) {
    ...
    for (const StorageError& error : storage->getErrors()) {
      for (auto errorLocationIt = errorLocationMap.find(error.id);
           errorLocationIt != errorLocationMap.end() && errorLocationIt->first == error.id; errorLocationIt++) {
        std::wstring locStr = addLocationStr(L"", errorLocationIt->second);
        testStorage->errors.emplace_back(error.message + locStr);
        testStorage->addLine(L"ERROR: " + error.message +
 ...
 private:
  ...
  void addLine(const std::wstring& message) { m_lines.emplace_back(utility::encodeToUtf8(message) + '\n'); }
};
