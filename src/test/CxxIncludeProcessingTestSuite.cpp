...
TEST_CASE("include detection finds include with quotes") {
  std::vector<IncludeDirective> includeDirectives =
      IncludeProcessing::getIncludeDirectives(TextAccess::createFromString("#include \"foo.h\"", FilePath(L"foo.cpp")));
...
TEST_CASE("header search path detection finds path inside sub directory") {
  ...
  REQUIRE(utility::containsElement<FilePath>(
      headerSearchDirectories, FilePath(L"data/CxxIncludeProcessingTestSuite/"
