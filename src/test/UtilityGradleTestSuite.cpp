...
TEST_CASE("gradle wrapper detects source directories of simple projects") {
  std::vector<FilePath> result =
      utility::gradleGetAllSourceDirectories(FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project"), false);

  REQUIRE(utility::containsElement<FilePath>(
      result, FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project/src/main/java").makeAbsolute()));

  REQUIRE(!utility::containsElement<FilePath>(
      result, FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project/src/test/java").makeAbsolute()));
}

TEST_CASE("gradle wrapper detects source and test directories of simple projects") {
  std::vector<FilePath> result =
      utility::gradleGetAllSourceDirectories(FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project"), true);
...
TEST_CASE("gradle wrapper detects source dependencies of simple projects") {
  ...
  cleanup();
  REQUIRE(utility::gradleCopyDependencies(FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project"),
                                          tmpFolder.makeAbsolute(), false));
  ...
  for (const std::wstring requiredDependency : requiredDependencies) {
    REQUIRE(utility::containsElement(copiedDependencies, tmpFolder.getConcatenated(requiredDependency)));
  }
  cleanup();
}

TEST_CASE("gradle wrapper detects source and test dependencies of simple projects") {
  std::vector<std::wstring> requiredDependencies = {L"joda-time-2.2.jar", L"hamcrest-core-1.3.jar", L"junit-4.12.jar"};

  cleanup();
  REQUIRE(utility::gradleCopyDependencies(FilePath(L"data/UtilityGradleTestSuite/simple_gradle_project"),
                                          tmpFolder.makeAbsolute(), true));
