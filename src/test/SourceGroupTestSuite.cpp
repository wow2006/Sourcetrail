...
std::shared_ptr<TextAccess> generateExpectedOutput(std::wstring projectName,
  ...
  std::sort(indexerCommands.begin(), indexerCommands.end(),
...
void generateAndCompareExpectedOutput(std::wstring projectName, std::shared_ptr<const SourceGroup> sourceGroup) {
  ...
  if (updateExpectedOutput || !expectedOutputFilePath.exists()) {
    ...
    REQUIRE_MESSAGE(("Output does not match the expected line count for project \"" +
...
TEST_CASE("source group cxx codeblocks generates expected output") {
...

  {
    ...
    fileStream << utility::replace(TextAccess::createFromFile(sourceCbpPath)->getText(), "<source_path>",
...
TEST_CASE("sourcegroup java empty generates expected output") {
  ...
  sourceGroupSettings->setClasspath({getInputDirectoryPath(projectName).concatenate(L"lib/dependency.jar"),
