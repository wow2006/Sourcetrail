...
class TestStorage : public PersistentStorage {
 public:
  TestStorage() : PersistentStorage(FilePath(L"data/test.sqlite"), FilePath(L"data/testBookmarks.sqlite")) { clear(); }
...
ParseLocation validLocation(Id locationId = 0) { return ParseLocation(1, 1, locationId, 1, locationId); }
...
TEST_CASE("storage saves file") {
  ...
  Id id = intermediateStorage
              ->addNode(StorageNodeData(nodeKindToInt(NODE_FILE),
