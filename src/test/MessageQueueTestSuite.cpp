...
class TestMessage : public Message<TestMessage> {
 public:
  static const std::string getStaticType() { return "TestMessage"; }
};

class Test2Message : public Message<Test2Message> {
 public:
  static const std::string getStaticType() { return "TestMessage2"; }
};
...
class TestMessageListener : public MessageListener<TestMessage> {
 ...
 private:
  virtual void handleMessage(TestMessage* message) { m_messageCount++; }
};
...
class Test3MessageListener : public MessageListener<Test2Message> {
 ...
 private:
  virtual void handleMessage(Test2Message* message) { m_listener = std::make_shared<TestMessageListener>(); }
};

class Test4MessageListener : public MessageListener<TestMessage>, public MessageListener<Test2Message> {
 public:
  std::shared_ptr<TestMessageListener> m_listener;
 ...
 private:
  ...
  virtual void handleMessage(Test2Message* message) { m_listener.reset(); }
};
