...
std::shared_ptr<TestStorage> parseCode(std::string code) {
...

  {
    ...
    std::shared_ptr<IndexerCommandCustom> indexerCommand = std::make_shared<IndexerCommandCustom>(
        ...
        FilePath("../app")
            ...
            .makeCanonical()
            .wstr(),
        args, rootPath, tempDbPath, std::to_wstring(SqliteIndexStorage::getStorageVersion()), sourceFilePath, true);
    ...
    if (!out.error.empty()) {
      FAIL("Error occurred while running the indexer: \"" + utility::encodeToUtf8(out.error) +
  ...
  {
    std::shared_ptr<PersistentStorage> persistentStorage = std::make_shared<PersistentStorage>(tempDbPath, FilePath());
...
TEST_CASE("python post processing regards class name in call context when adding ambiguous edges") {
  ...
  REQUIRE(utility::containsElement<std::wstring>(storage->calls, L"test.A1.__init__ -> test.A.__init__"));

  REQUIRE(!utility::containsElement<std::wstring>(storage->calls, L"test.A1.__init__ -> test.A1.__init__"));
  REQUIRE(!utility::containsElement<std::wstring>(storage->calls, L"test.A1.__init__ -> test.B.__init__"));
}
