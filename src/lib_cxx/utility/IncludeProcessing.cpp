...
std::vector<IncludeDirective> IncludeProcessing::getUnresolvedIncludeDirectives(
    const std::set<FilePath>& sourceFilePaths, const std::set<FilePath>& indexedPaths,
    const std::set<FilePath>& headerSearchDirectories, const size_t desiredQuantileCount,
  ...
  std::vector<std::vector<FilePath>> parts =
      utility::splitToEquallySizedParts(utility::toVector(sourceFilePaths), desiredQuantileCount);
  ...
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    std::copy(directives.begin(), directives.end(),
...
std::set<FilePath> IncludeProcessing::getHeaderSearchDirectories(
    const std::set<FilePath>& sourceFilePaths, const std::set<FilePath>& searchedPaths,
    const std::set<FilePath>& currentHeaderSearchDirectories, const size_t desiredQuantileCount,
    std::function<void(float)> progress) {
  progress(0.0f);
  ...
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    while (!unprocessedFilePaths.empty()) {
      std::transform(unprocessedFilePaths.begin(), unprocessedFilePaths.end(),
  ...
  for (unsigned i = 0; i < lines.size(); i++) {
    ...
    if (utility::isPrefix<std::wstring>(L"#", lineTrimmedToHash)) {
      ...
      if (utility::isPrefix<std::wstring>(L"include", lineTrimmedToInclude)) {
        std::wstring includeString = utility::substrBetween<std::wstring>(lineTrimmedToInclude, L"<", L">");
        bool usesBrackets = true;
        if (includeString.empty()) {
          includeString = utility::substrBetween<std::wstring>(lineTrimmedToInclude, L"\"", L"\"");
          usesBrackets = false;
        }

        if (!includeString.empty()) {
          // lines are 1 based
          includeDirectives.push_back(
              IncludeDirective(FilePath(includeString), textAccess->getFilePath(), i + 1, usesBrackets));
        }
      }
    }
  }
...
std::vector<IncludeDirective> IncludeProcessing::doGetUnresolvedIncludeDirectives(
  ...
  while (!filePathsToProcess.empty()) {
    std::transform(filePathsToProcess.begin(), filePathsToProcess.end(),
                   std::inserter(processedFilePaths, processedFilePaths.begin()),
...
FilePath IncludeProcessing::resolveIncludeDirective(const IncludeDirective& includeDirective,
