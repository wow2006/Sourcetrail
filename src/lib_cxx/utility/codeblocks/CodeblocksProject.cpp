...
std::shared_ptr<Project> Project::load(std::shared_ptr<TextAccess> xmlAccess) {
  ...
  if (doc.Error()) {
    LOG_ERROR("Unable to parse Code::Blocks project because of an error in row " + std::to_string(doc.ErrorRow()) +
              ", col " + std::to_string(doc.ErrorCol()) + ": " + std::string(doc.ErrorDesc()));
    return project;
  }
...
std::set<FilePath> Project::getAllSourceFilePathsCanonical(const std::vector<std::wstring>& sourceExtensions) const {
...
std::vector<std::shared_ptr<IndexerCommandCxx>> Project::getIndexerCommands(
  ...
  const std::vector<FilePath> frameworkSearchPaths =
      utility::concat(sourceGroupSettings->getFrameworkSearchPathsExpandedAndAbsolute(),
  ...
  for (std::shared_ptr<Unit> unit : m_units) {
    ...
    if (!unit->getTargetNames().size()) {
      nonTargetIndexerCommands.push_back(std::make_shared<IndexerCommandCxx>(
          filePath, utility::concat(indexedHeaderPaths, {filePath}), excludeFilters, std::set<FilePathFilter>(),
