...
CxxAstVisitorComponentIndexer::CxxAstVisitorComponentIndexer(CxxAstVisitor* astVisitor, clang::ASTContext* astContext,
      ...
      if (templateTemplateArgumentName.isDependent()) {
...

        {
          if (const clang::NamedDecl* namedContextDecl =
                  getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getTopmostContextDecl(1)) {
            m_client->recordReference(REFERENCE_TYPE_USAGE, symbolId,
...
void CxxAstVisitorComponentIndexer::beginTraverseLambdaCapture(clang::LambdaExpr* lambdaExpr,
...
void CxxAstVisitorComponentIndexer::visitTagDecl(clang::TagDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    if (clang::EnumDecl* enumDecl = clang::dyn_cast_or_null<clang::EnumDecl>(d)) {
      recordTemplateMemberSpecialization(enumDecl->getMemberSpecializationInfo(), symbolId, location, symbolKind);
    }

    if (clang::CXXRecordDecl* recordDecl = clang::dyn_cast_or_null<clang::CXXRecordDecl>(d)) {
      recordTemplateMemberSpecialization(recordDecl->getMemberSpecializationInfo(), symbolId, location, symbolKind);
    }
  }
}
...
void CxxAstVisitorComponentIndexer::visitClassTemplateSpecializationDecl(clang::ClassTemplateSpecializationDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    m_client->recordReference(REFERENCE_TEMPLATE_SPECIALIZATION, getOrCreateSymbolId(specializedFromDecl),
                              getOrCreateSymbolId(d), getParseLocation(d->getLocation()));
  }
}
    ...
    if (clang::CXXRecordDecl* declaringRecordDecl = clang::dyn_cast_or_null<clang::CXXRecordDecl>(d->getParent())) {
      if (clang::CXXRecordDecl* declaringRecordTemplateDecl = declaringRecordDecl->getTemplateInstantiationPattern()) {
        for (clang::FieldDecl* templateFieldDecl : declaringRecordTemplateDecl->fields()) {
          if (d->getDeclName().isIdentifier() && templateFieldDecl->getDeclName().isIdentifier() &&
            ...
            m_client->recordReference(REFERENCE_TEMPLATE_SPECIALIZATION, templateFieldId, fieldId, location);
            break;
          }
        }
      }
  ...
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    if (d->isFunctionTemplateSpecialization()) {
      if (clang::isa<clang::ClassTemplateSpecializationDecl>(d->getParent()) &&
          ...
          if (clang::CXXRecordDecl* declaringRecordTemplateDecl =
            ...
            for (clang::Decl* templateMethodDecl : declaringRecordTemplateDecl->decls()) {
              if (clang::FunctionTemplateDecl* functionTemplateDecl =
                      clang::dyn_cast_or_null<clang::FunctionTemplateDecl>(templateMethodDecl)) {
        ...
        m_client->recordReference(REFERENCE_TEMPLATE_SPECIALIZATION, templateId, symbolId,
                                  getParseLocation(d->getLocation()));
      }
    }
  }
...
void CxxAstVisitorComponentIndexer::visitCXXMethodDecl(clang::CXXMethodDecl* d) {
  ...
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    for (clang::CXXMethodDecl::method_iterator it = d->begin_overridden_methods();  // TODO: iterate in traversal and
                                                                                    // use REFERENCE_OVERRIDE or so..
    ...
    recordTemplateMemberSpecialization(d->getMemberSpecializationInfo(), symbolId, location, SYMBOL_FUNCTION);
  }
}
...
void CxxAstVisitorComponentIndexer::visitNamespaceAliasDecl(clang::NamespaceAliasDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    m_client->recordReference(REFERENCE_USAGE, getOrCreateSymbolId(d->getAliasedNamespace()), symbolId,
...
void CxxAstVisitorComponentIndexer::visitTypedefDecl(clang::TypedefDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    Id symbolId = getOrCreateSymbolId(d);
    m_client->recordSymbolKind(symbolId, d->getAnonDeclWithTypedefName() == nullptr
                                             ? SYMBOL_TYPEDEF
...
void CxxAstVisitorComponentIndexer::visitUsingDirectiveDecl(clang::UsingDirectiveDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    m_client->recordReference(
        REFERENCE_USAGE, symbolId,
        getOrCreateSymbolId(
            getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getContext(),
            NameHierarchy(getAstVisitor()->getCanonicalFilePathCache()->getCanonicalFilePath(location.fileId).wstr(),
                          NAME_DELIMITER_FILE)),
        location);
  }
}
...
void CxxAstVisitorComponentIndexer::visitUsingDecl(clang::UsingDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    ...
    m_client->recordReference(
        REFERENCE_USAGE, getOrCreateSymbolId(d),
        getOrCreateSymbolId(
            getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getContext(),
            NameHierarchy(getAstVisitor()->getCanonicalFilePathCache()->getCanonicalFilePath(location.fileId).wstr(),
                          NAME_DELIMITER_FILE)),
...
void CxxAstVisitorComponentIndexer::visitTypeLoc(clang::TypeLoc tl) {
  ...
  if ((getAstVisitor()->shouldVisitReference(tl.getBeginLoc())) && (!getAstVisitor()->checkIgnoresTypeLoc(tl))) {
    if (!tl.getAs<clang::TemplateTypeParmTypeLoc>().isNull()) {
      ...
      if (decl) {
        m_client->recordLocalSymbol(getLocalSymbolName(decl->getLocation()), getParseLocation(tl.getBeginLoc()));
      }
    } else {
      ...
      if (getAstVisitor()->getComponent<CxxAstVisitorComponentTypeRefKind>()->isTraversingTemplateArgument()) {
        if (const clang::NamedDecl* namedContextDecl =
                getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getTopmostContextDecl(2)) {
          m_client->recordReference(REFERENCE_TYPE_USAGE, symbolId,
...
void CxxAstVisitorComponentIndexer::visitDeclRefExpr(clang::DeclRefExpr* s) {
  ...
  if (getAstVisitor()->shouldVisitReference(s->getLocation())) {
    ...
    } else if ((clang::isa<clang::NonTypeTemplateParmDecl>(decl)) || (clang::isa<clang::TemplateTypeParmDecl>(decl)) ||
...
void CxxAstVisitorComponentIndexer::visitLambdaExpr(clang::LambdaExpr* s) {
  ...
  if (getAstVisitor()->shouldVisitDecl(methodDecl)) {
    ...
    m_client->recordDefinitionKind(symbolId,
...
void CxxAstVisitorComponentIndexer::visitConstructorInitializer(clang::CXXCtorInitializer* init) {
  if (getAstVisitor()->shouldVisitReference(init->getMemberLocation())) {
    ...
    if (clang::FieldDecl* memberDecl = init->getMember()) {
      m_client->recordReference(
          REFERENCE_USAGE, getOrCreateSymbolId(memberDecl),
          getOrCreateSymbolId(getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getContext()),
...
void CxxAstVisitorComponentIndexer::recordTemplateMemberSpecialization(
    const clang::MemberSpecializationInfo* memberSpecializationInfo, Id contextId, const ParseLocation& location,
...
std::wstring CxxAstVisitorComponentIndexer::getLocalSymbolName(const clang::SourceLocation& loc) const {
  ...
  return getAstVisitor()->getCanonicalFilePathCache()->getCanonicalFilePath(location.fileId).fileName() + L"<" +
         std::to_wstring(location.startLineNumber) + L":" + std::to_wstring(location.startColumnNumber) + L">";
}
  ...
  if (decl) {
    ...
    if (declName) {
      ...
      if (dynamic_cast<CxxFunctionDeclName*>(declName.get()) && symbolName.size() == 1 &&
        ...
        symbolName.pop();
        symbolName.push(
            NameElement(L".:main:." + getAstVisitor()->getCanonicalFilePathCache()->getDeclarationFilePath(decl).wstr(),
                        sig.getPrefix(), sig.getPostfix()));
      }
    }
  }
