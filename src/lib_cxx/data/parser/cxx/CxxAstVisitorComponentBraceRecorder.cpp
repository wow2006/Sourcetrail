...
void CxxAstVisitorComponentBraceRecorder::visitTagDecl(clang::TagDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    if (d->isThisDeclarationADefinition() &&
      ...
      recordBraces(getFilePath(d->getBraceRange().getBegin()), getParseLocation(d->getBraceRange().getBegin()),
...
void CxxAstVisitorComponentBraceRecorder::visitNamespaceDecl(clang::NamespaceDecl* d) {
  if (getAstVisitor()->shouldVisitDecl(d)) {
    recordBraces(getFilePath(d->getBeginLoc()),
...
void CxxAstVisitorComponentBraceRecorder::visitCompoundStmt(clang::CompoundStmt* s) {
  if (getAstVisitor()->shouldVisitStmt(s)) {
    ...
    if (!contextDecl || !utility::isImplicit(contextDecl)) {
      recordBraces(getFilePath(s->getLBracLoc()), getParseLocation(s->getLBracLoc()),
  ...
  if (getAstVisitor()->shouldVisitStmt(s)) {
    if (s->isSyntacticForm()) {
      const clang::NamedDecl* contextDecl =
          getAstVisitor()->getComponent<CxxAstVisitorComponentContext>()->getTopmostContextDecl();
      if (!contextDecl || !utility::isImplicit(contextDecl)) {
        recordBraces(getFilePath(s->getLBraceLoc()), getParseLocation(s->getLBraceLoc()),
...
void CxxAstVisitorComponentBraceRecorder::recordBraces(const FilePath& filePath, const ParseLocation& lbraceLoc,
                                                       const ParseLocation& rbraceLoc) {
  if (lbraceLoc.startColumnNumber != rbraceLoc.startColumnNumber ||
      lbraceLoc.endColumnNumber != rbraceLoc.endColumnNumber ||
      lbraceLoc.startLineNumber != rbraceLoc.startLineNumber || lbraceLoc.endLineNumber != rbraceLoc.endLineNumber) {
    std::wstring name = filePath.fileName() + L"<" + std::to_wstring(lbraceLoc.startLineNumber) + L":" +
