...
std::shared_ptr<FileRegister> CanonicalFilePathCache::getFileRegister() const { return m_fileRegister; }

FilePath CanonicalFilePathCache::getCanonicalFilePath(const clang::FileID& fileId,
...
FilePath CanonicalFilePathCache::getDeclarationFilePath(const clang::Decl* declaration) {
  ...
  return getCanonicalFilePath(
      utility::decodeFromUtf8(sourceManager.getPresumedLoc(declaration->getBeginLoc()).getFilename()));
}
