...
CxxStaticFunctionDeclName::CxxStaticFunctionDeclName(std::wstring name,
    ...
    : CxxFunctionDeclName(std::move(name), std::move(templateParameterNames), std::move(returnTypeName),
                          std::move(parameterTypeNames), false, true),
...
NameHierarchy CxxStaticFunctionDeclName::toNameHierarchy() const {
  ...
  last.setSignature(last.getSignature().getPrefix(),
