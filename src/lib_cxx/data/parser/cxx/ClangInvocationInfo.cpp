...
clang::driver::Driver* newDriver(clang::DiagnosticsEngine* Diagnostics, const char* BinaryName,
  ...
  clang::driver::Driver* CompilerDriver = new clang::driver::Driver(BinaryName, llvm::sys::getDefaultTargetTriple(),
                                                                    *Diagnostics, "clang_based_tool", std::move(VFS));
#else
...
ClangInvocationInfo ClangInvocationInfo::getClangInvocationString(
  ...
  if (!compilationDatabase->getAllCompileCommands().empty()) {
    ...
    for (const std::string& Str : CommandLine) Argv.push_back(Str.c_str());
