...
bool utility::isParameter(const clang::VarDecl* d) { return llvm::isa<clang::ParmVarDecl>(d); }
...
ParseLocation utility::getParseLocation(const clang::SourceLocation& sourceLocation,
                                        const clang::SourceManager& sourceManager, clang::Preprocessor* preprocessor,
  ...
  if (sourceLocation.isValid()) {
    ...
    if (preprocessor != nullptr) {
      ...
      return ParseLocation(
          canonicalFilePathCache->getFileSymbolId(fileId), sourceManager.getLineNumber(fileId, startOffset),
          sourceManager.getColumnNumber(fileId, startOffset), sourceManager.getLineNumber(fileId, endOffset),
          sourceManager.getColumnNumber(fileId, endOffset) - 1);
    } else {
      return ParseLocation(canonicalFilePathCache->getFileSymbolId(fileId),
...
ParseLocation utility::getParseLocation(const clang::SourceRange& sourceRange,
                                        const clang::SourceManager& sourceManager, clang::Preprocessor* preprocessor,
                                        std::shared_ptr<CanonicalFilePathCache> canonicalFilePathCache) {
  if (sourceRange.isValid()) {
    ...
    const clang::PresumedLoc presumedEnd =
        sourceManager.getPresumedLoc(endLoc.isValid() ? endLoc : range.getEnd(), false);
