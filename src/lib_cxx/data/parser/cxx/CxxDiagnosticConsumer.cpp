...
CxxDiagnosticConsumer::CxxDiagnosticConsumer(clang::raw_ostream& os, clang::DiagnosticOptions* diags,
                                             std::shared_ptr<ParserClient> client,
                                             std::shared_ptr<CanonicalFilePathCache> canonicalFilePathCache,
                                             const FilePath& sourceFilePath, bool useLogging)
...
void CxxDiagnosticConsumer::BeginSourceFile(const clang::LangOptions& langOptions,
...
void CxxDiagnosticConsumer::HandleDiagnostic(clang::DiagnosticsEngine::Level level, const clang::Diagnostic& info) {
  ...
  if (level >= clang::DiagnosticsEngine::Error) {
    ...
    if (fileId != 0) {
      m_client->recordError(utility::decodeFromUtf8(message), level == clang::DiagnosticsEngine::Fatal,
                            m_canonicalFilePathCache->getFileRegister()->hasFilePath(filePath), m_sourceFilePath,
