...
PreprocessorCallbacks::PreprocessorCallbacks(clang::SourceManager& sourceManager, std::shared_ptr<ParserClient> client,
                                             std::shared_ptr<CanonicalFilePathCache> canonicalFilePathCache)
    : m_sourceManager(sourceManager), m_client(client), m_canonicalFilePathCache(canonicalFilePathCache) {}

void PreprocessorCallbacks::FileChanged(clang::SourceLocation location, FileChangeReason reason,
                                        clang::SrcMgr::CharacteristicKind, clang::FileID prevID) {
...
void PreprocessorCallbacks::InclusionDirective(clang::SourceLocation hashLocation, const clang::Token& includeToken,
  ...
  if (m_currentFileSymbolId && fileEntry) {
    ...
    m_client->recordReference(REFERENCE_INCLUDE, includedFileSymbolId, m_currentFileSymbolId,
                              getParseLocation(fileNameRange.getAsRange()));
  }
}

void PreprocessorCallbacks::MacroDefined(const clang::Token& macroNameToken,
  ...
  if (m_currentPathIsProjectFile) {
    ...
    const NameHierarchy nameHierarchy(utility::decodeFromUtf8(macroNameToken.getIdentifierInfo()->getName().str()),
                                      NAME_DELIMITER_CXX);
...
void PreprocessorCallbacks::MacroUndefined(const clang::Token& macroNameToken,
                                           const clang::MacroDefinition& macroDefinition,
                                           const clang::MacroDirective* macroUndefinition) {
  onMacroUsage(macroNameToken);
}

void PreprocessorCallbacks::Defined(const clang::Token& macroNameToken, const clang::MacroDefinition& macroDefinition,
...
void PreprocessorCallbacks::MacroExpands(const clang::Token& macroNameToken,
                                         const clang::MacroDefinition& macroDirective, clang::SourceRange range,
...
ParseLocation PreprocessorCallbacks::getParseLocation(const clang::Token& macroNameTok) const {
  ...
  if (fileSymbolId) {
    return ParseLocation(fileSymbolId, m_sourceManager.getSpellingLineNumber(location),
...
ParseLocation PreprocessorCallbacks::getParseLocation(const clang::SourceRange& sourceRange) const {
  if (sourceRange.isValid()) {
    ...
    Id fileSymbolId = m_canonicalFilePathCache->getFileSymbolId(m_sourceManager.getFileID(sourceRange.getBegin()));

    if (fileSymbolId) {
      return ParseLocation(fileSymbolId, presumedBegin.getLine(), presumedBegin.getColumn(), presumedEnd.getLine(),
