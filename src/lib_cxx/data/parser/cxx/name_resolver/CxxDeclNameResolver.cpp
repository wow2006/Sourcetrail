...
std::unique_ptr<CxxDeclName> CxxDeclNameResolver::getName(const clang::NamedDecl* declaration) {
  ...
  if (declName) {
    ...
    } else if (!clang::isa<clang::TemplateTypeParmDecl>(declaration) &&
...
std::unique_ptr<CxxDeclName> CxxDeclNameResolver::getDeclName(const clang::NamedDecl* declaration) {
  if (declaration) {
    ...
    if (const clang::RecordDecl* recordDecl = clang::dyn_cast_or_null<clang::RecordDecl>(declaration)) {
      if (recordDecl->isLambda()) {
      ...
      } else if (const clang::CXXRecordDecl* cxxRecordDecl =
                     clang::dyn_cast_or_null<clang::CXXRecordDecl>(declaration)) {
        ...
        } else if (const clang::ClassTemplateSpecializationDecl* templateSpecialitarionDecl =
      ...
      if ((clang::dyn_cast_or_null<clang::CXXMethodDecl>(functionDecl)) &&
        ...
        const clang::PresumedLoc& presumedBegin = sourceManager.getPresumedLoc(
            clang::dyn_cast_or_null<clang::CXXMethodDecl>(functionDecl)->getParent()->getBeginLoc());
        functionName = L"lambda at " + std::to_wstring(presumedBegin.getLine()) + L":" +
                       std::to_wstring(presumedBegin.getColumn());
      } else if (clang::FunctionTemplateDecl* templateFunctionDeclaration =
      ...
      if (!clang::isa<clang::CXXMethodDecl>(declaration) && isStatic) {
        return std::make_unique<CxxStaticFunctionDeclName>(std::move(functionName), std::move(templateArguments),
                                                           std::move(returnTypeName), std::move(parameterTypeNames),
                                                           getTranslationUnitMainFileName(declaration));
      }

      return std::make_unique<CxxFunctionDeclName>(std::move(functionName), std::move(templateArguments),
                                                   std::move(returnTypeName), std::move(parameterTypeNames), isConst,
                                                   isStatic);
      ...
      std::unique_ptr<CxxTypeName> typeName =
          CxxTypeName::makeUnsolvedIfNull(typenNameResolver.getName(fieldDecl->getType()));
      return std::make_unique<CxxVariableDeclName>(std::move(declNameString), std::vector<std::wstring>(),
                                                   std::move(typeName), false);
      ...
      if (varDecl->getParentFunctionOrMethod() == nullptr) {
        ...
        std::unique_ptr<CxxTypeName> typeName =
            CxxTypeName::makeUnsolvedIfNull(typenNameResolver.getName(varDecl->getType()));
        ...
        return std::make_unique<CxxVariableDeclName>(std::move(varName), std::move(templateParameterNames),
    ...
    {
      return std::make_unique<CxxDeclName>(
          std::move(declNameString), getTemplateParameterStrings(clang::dyn_cast<clang::TemplateDecl>(declaration)));
    }
...
std::wstring CxxDeclNameResolver::getNameForAnonymousSymbol(const std::wstring& symbolKindName,
                                                            const clang::Decl* declaration) {
  const clang::SourceManager& sourceManager = declaration->getASTContext().getSourceManager();
  const clang::PresumedLoc& presumedBegin = sourceManager.getPresumedLoc(declaration->getBeginLoc());

  if (presumedBegin.isValid()) {
    return L"anonymous " + symbolKindName + L" (" + getCanonicalFilePathCache()->getDeclarationFileName(declaration) +
           L'<' + std::to_wstring(presumedBegin.getLine()) + L':' + std::to_wstring(presumedBegin.getColumn()) + L">)";
  }
