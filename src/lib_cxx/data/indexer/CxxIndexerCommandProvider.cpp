...
void CxxIndexerCommandProvider::addCommand(const std::shared_ptr<IndexerCommandCxx>& command) {
  ...
  {
    for(const utility::file::FilePath& indexedPath: command->getIndexedPaths()) {
      std::map<utility::file::FilePath, Id>::const_iterator it = m_indexedPathsToIds.find(indexedPath);
  ...
  {
    const utility::file::FilePath workingDirectory = command->getWorkingDirectory();
    std::map<utility::file::FilePath, Id>::const_iterator it = m_workingDirectoriesToIds.find(workingDirectory);
...
std::vector<utility::file::FilePath> CxxIndexerCommandProvider::getAllSourceFilePaths() const {
  std::vector<utility::file::FilePath> paths;
  paths.reserve(m_commands.size());

  for(std::map<utility::file::FilePath, std::shared_ptr<CommandRepresentation>>::const_iterator it =
          m_commands.begin();
      it != m_commands.end();
      it++) {
...
std::shared_ptr<IndexerCommand> CxxIndexerCommandProvider::consumeCommand() {
  if(!m_commands.empty()) {
    std::map<utility::file::FilePath, std::shared_ptr<CommandRepresentation>>::const_iterator it = m_commands.begin();
    if(it->second) {
...
std::shared_ptr<IndexerCommand> CxxIndexerCommandProvider::consumeCommandForSourceFilePath(
  ...
  std::map<utility::file::FilePath, std::shared_ptr<CommandRepresentation>>::const_iterator it = m_commands.find(
      filePath);
...
std::shared_ptr<IndexerCommandCxx> CxxIndexerCommandProvider::representationToCommand(
  ...
  utility::file::FilePath workingDirectory = m_idsToWorkingDirectories[representation->m_workingDirectoryId];
