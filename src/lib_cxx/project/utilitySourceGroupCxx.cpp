...
std::shared_ptr<Task> createBuildPchTask(const SourceGroupSettingsWithCxxPchOptions* settings,
  ...
  return std::make_shared<TaskLambda>([dialogView, storageProvider, pchInputFilePath, pchOutputFilePath,
                                       compilerFlags]() {
    dialogView->showUnknownProgressDialog(L"Preparing Indexing", L"Processing Precompiled Headers");
    LOG_INFO(L"Generating precompiled header output for input file \"" + pchInputFilePath.wstr() +
...
std::shared_ptr<clang::tooling::JSONCompilationDatabase> loadCDB(const FilePath& cdbPath, std::string* error) {
  ...
  std::shared_ptr<clang::tooling::JSONCompilationDatabase> cdb =
      std::shared_ptr<clang::tooling::JSONCompilationDatabase>(clang::tooling::JSONCompilationDatabase::loadFromFile(
...
void removeIncludePchFlag(std::vector<std::wstring>& args) {
  ...
  for (size_t i = 0; i < args.size(); i++) {
    ...
    if (utility::isPrefix<std::wstring>(includePchPrefix, arg)) {
      if (i + 1 < args.size() && !utility::isPrefix<std::wstring>(L"-", utility::trim(args[i + 1])) &&
